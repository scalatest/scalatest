  ...
  describe("A PosZLong") {
    describe("should offer a from factory method that") {
      it("returns Some[PosZLong] if the passed Long is greater than or equal to 0") {
        PosZLong.from(0L) shouldBe Some(0L)
        PosZLong.from(50L) shouldBe Some(50L)
        PosZLong.from(100L) shouldBe Some(100L)
      }
    ...
    describe("should offer an ensuringValid factory method that") {
      it("returns PosZLong if the passed Long is greater than or equal to 0") {
        PosZLong.ensuringValid(0L) shouldBe PosZLong(0L)
        PosZLong.ensuringValid(50L) shouldBe PosZLong(50L)
        PosZLong.ensuringValid(100L) shouldBe PosZLong(100L)
      }
    ...
    it("should not offer widening methods for basic types that are consistent with Long") {
      ...
      forAll { (pzlong: PosZLong) =>
        ...
        "widen(pzlong) shouldEqual widen(pzlong.toLong)" shouldNot compile
        succeed
      }
      forAll { (pzlong: PosZLong) =>
