  ...
  describe("A PosZFloat") {
    describe("should offer a from factory method that") {
      it("returns Some[PosZFloat] if the passed Float is greater than or equal to 0") {
        PosZFloat.from(0.0f) shouldBe Some(PosZFloat(0.0f))
        PosZFloat.from(50.23f) shouldBe Some(PosZFloat(50.23f))
        PosZFloat.from(100.0f) shouldBe Some(PosZFloat(100.0f))
      }
    ...
    describe("should offer an ensuringValid factory method that") {
      it("returns PosZFloat if the passed Float is greater than or equal to 0") {
        PosZFloat.ensuringValid(0.0f) shouldBe PosZFloat(0.0f)
        PosZFloat.ensuringValid(50.23f) shouldBe PosZFloat(50.23f)
        PosZFloat.ensuringValid(100.0f) shouldBe PosZFloat(100.0f)
