  ...
  describe("A PosZDouble") {
    describe("should offer a from factory method that") {
      it("returns Some[PosZDouble] if the passed Double is greater than or equal to 0") {
        PosZDouble.from(0.0) shouldBe Some(PosZDouble(0.0))
        PosZDouble.from(50.23) shouldBe Some(PosZDouble(50.23))
        PosZDouble.from(100.0) shouldBe Some(PosZDouble(100.0))
      }
