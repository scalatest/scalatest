...
class NegLongSpec extends funspec.AnyFunSpec with matchers.should.Matchers with GeneratorDrivenPropertyChecks with NegLongSpecSupport {
  ...
  describe("A NegLong") {
    ...
    describe("when specified as a plain-old Long") {
      ...
      it("should compile when -8 is passed in") {
        "takesNegLong(NegLong(-8L))" should compile
        takesNegLong(NegLong(-8L)) shouldEqual -8L
        "takesNegLong(NegLong(-8))" should compile
        takesNegLong(NegLong(-8)) shouldEqual -8L
      }

      it("should not compile when 0 is passed in") {
        "takesNegLong(NegLong(0))" shouldNot compile
        "takesNegLong(NegLong(0L))" shouldNot compile
      }

      it("should not compile when 8 is passed in") {
        "takesNegLong(NegLong(8))" shouldNot compile
        "takesNegLong(NegLong(8L))" shouldNot compile
      }

      it("should not compile when x is passed in") {
        val x: Int = -8
        "takesNegLong(NegLong(x))" shouldNot compile
        val b: Long = -8L
        "takesNegLong(NegLong(b))" shouldNot compile
      }
