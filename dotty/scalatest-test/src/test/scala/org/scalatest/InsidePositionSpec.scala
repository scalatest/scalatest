...
class InsidePositionSpec extends AnyFunSpec with Matchers with Inside {

  def insideShouldMatch[T, U](value: T)(pf: PartialFunction[T, U])(using pos: source.Position): U = {
    inside(value)(pf)
  }
  ...
  describe("inside") {
    it("should use the Position available in the enclosing scope when reporting failure location") {
      ...
      val e =
        try {
          insideShouldMatch(List(1, 2, 3)) { case List(x) => x } (using callerPos)
