...
class EveryShouldContainAtMostOneElementOfLogicalOrSpec extends AnyFreeSpec {
  ...
  "every of Everys" - {
    ...
    "when used with (be (...) and contain oneOf (...)) syntax" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (be (Many(1, 2)) or contain atMostOneElementOf Seq(1, 6, 8))
        all (list1s) should (be (Many(2, 3)) or contain atMostOneElementOf Seq(1, 6, 8))
        all (list1s) should (be (Many(1, 2)) or contain atMostOneElementOf Seq(1, 2, 3))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (be (Many(2, 3)) or contain atMostOneElementOf Seq(1, 2, 3))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(prettifier, list1s(0)) + " was not equal to " + decorateToStringValue(prettifier, Many(2, 3)) + ", and " + decorateToStringValue(prettifier, list1s(0)) + " did not contain at most one element of " + decorateToStringValue(prettifier, Seq(1, 2, 3)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      "should use the implicit Equality in scope" in {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (be (Many("hi", "he")) or contain atMostOneElementOf Seq("HI", "HO"))
        all (hiLists) should (be (One("HO")) or contain atMostOneElementOf Seq("HI", "HO"))
        all (hiLists) should (be (Many("hi", "he")) or contain atMostOneElementOf Seq("HI", "HE"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be (One("HO")) or contain atMostOneElementOf Seq("HI", "HE"))
        }
      ...
      "should do nothing when RHS contain duplicated value" in {
        all (list1s) should (be (Many(1, 2)) or contain atMostOneElementOf Seq(1, 2, 2, 3))
      }
    }
    ...
    "when used with (not be (...) and not contain atMostOneElementOf Seq(...))" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (not be (Many(2, 3)) or not contain atMostOneElementOf (Seq(1, 2, 3)))
        all (list1s) should (not be (Many(1, 2)) or not contain atMostOneElementOf (Seq(1, 2, 3)))
        all (list1s) should (not be (Many(2, 3)) or not contain atMostOneElementOf (Seq(2, 3, 4)))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (not be (Many(1, 2)) or not contain atMostOneElementOf (Seq(2, 3, 4)))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(prettifier, list1s(0)) + " was equal to " + decorateToStringValue(prettifier, Many(1, 2)) + ", and " + decorateToStringValue(prettifier, list1s(0)) + " contained at most one element of " + decorateToStringValue(prettifier, Seq(2, 3, 4)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      "should use the implicit Equality in scope" in {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (not be (One("ho")) or not contain atMostOneElementOf (Seq("HI", "HE")))
        all (hiLists) should (not be (Many("hi", "he")) or not contain atMostOneElementOf (Seq("HE", "HI")))
        all (hiLists) should (not be (One("ho")) or not contain atMostOneElementOf (Seq("HI", "HE")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be (Many("hi", "he")) or not contain atMostOneElementOf (Seq("HI", "HO")))
        }
      ...
      "should do nothing when RHS contain duplicated value" in {
        all (list1s) should (not be (Many(2, 3)) or not contain atMostOneElementOf (Seq(1, 2, 2, 3)))
      }
    }
  }
}
