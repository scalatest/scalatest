...
class ThreadNameSpec extends AnyFunSpec {
  ...
  describe("The Thread name augmenter") {
    ...
    it("""should replace a string that does not include "ScalaTest-" with "<exising thread name>-ScalaTest-running-Z"""") {
      ...
      SuiteHelpers.augmentedThreadName("something-else-running-something", "Elephant") shouldBe "something-else-running-something-ScalaTest-running-Elephant"
    }
  }
}
