...
class EveryShouldContainAtMostOneElementOfLogicalAndSpec extends AnyFunSpec {
  ...
  describe("every of Everys") {
    ...
    describe("when used with (be (..) and contain atMostOneElementOf Seq(..))") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        all (list1s) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(1, 3, 4))
        atLeast (2, lists) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(2, 3, 4))
        atMost (2, lists) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(2, 3, 4))
        no (lists) should (be (One(8)) and contain atMostOneElementOf Seq(3, 2, 1))

        val e1 = intercept[TestFailedException] {
          all (lists) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(1, 3, 4))
        }
        checkMessageStackDepth(e1, allErrMsg(2, decorateToStringValue(prettifier, lists(2)) + " was not equal to " + decorateToStringValue(prettifier, Many(1, 2)), thisLineNumber - 2, lists), fileName, thisLineNumber - 2)

        val e2 = intercept[TestFailedException] {
          all (list1s) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(1, 2, 3))
        }
        checkMessageStackDepth(e2, allErrMsg(0, decorateToStringValue(prettifier, list1s(0)) + " was equal to " + decorateToStringValue(prettifier, Many(1, 2)) + ", but " + decorateToStringValue(prettifier, list1s(0)) + " did not contain at most one element of " + decorateToStringValue(prettifier, Seq(1, 2, 3)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)

        val e4 = intercept[TestFailedException] {
          all (hiLists) should (be (Many("hi", "he")) and contain atMostOneElementOf Seq("hi", "he"))
        }
        checkMessageStackDepth(e4, allErrMsg(0, decorateToStringValue(prettifier, hiLists(0)) + " was equal to " + decorateToStringValue(prettifier, Many("hi", "he")) + ", but " + decorateToStringValue(prettifier, hiLists(0)) + " did not contain at most one element of " + decorateToStringValue(prettifier, Seq("hi", "he")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)

        val e6 = intercept[TestFailedException] {
          all (list1s) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(1, 2, 3))
        }
        checkMessageStackDepth(e6, allErrMsg(0, decorateToStringValue(prettifier, list1s(0)) + " was equal to " + decorateToStringValue(prettifier, Many(1, 2)) + ", but " + decorateToStringValue(prettifier, list1s(0)) + " did not contain at most one element of " + decorateToStringValue(prettifier, Seq(1, 2, 3)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      it("should use the implicit Equality in scope") {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (be (Many("hi", "he")) and contain atMostOneElementOf Seq("HO", "HE"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be (One("ho")) and contain atMostOneElementOf Seq("HO", "HE"))
        }
      ...
      it("should do nothing when RHS contain duplicated value") {
        all (list1s) should (be (Many(1, 2)) and contain atMostOneElementOf Seq(1, 3, 3, 8))
      }
    }
    ...
    describe("when used with (not be (..) and not contain atMostOneElementOf Seq(..))") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        all (list1s) should (not be (Many(2, 3)) and not contain atMostOneElementOf (Seq(1, 2, 3)))
        atLeast (2, lists) should (not be (Many(2, 3)) and not contain atMostOneElementOf (Seq(1, 2, 3)))
        atMost (2, lists) should (not be (Many(2, 3)) and contain atMostOneElementOf (Seq(1, 2, 3)))
        no (list1s) should (not be (Many(1, 2)) and not contain atMostOneElementOf (Seq(3, 6, 8)))

        val e1 = intercept[TestFailedException] {
          all (lists) should (not be (Many(2, 3)) and not contain atMostOneElementOf (Seq(1, 2, 3)))
        }
        checkMessageStackDepth(e1, allErrMsg(2, decorateToStringValue(prettifier, lists(2)) + " was equal to " + decorateToStringValue(prettifier, Many(2, 3)), thisLineNumber - 2, lists), fileName, thisLineNumber - 2)

        val e2 = intercept[TestFailedException] {
          all (lists) should (not be (Many(2, 3, 4)) and not contain atMostOneElementOf (Seq(1, 2, 8)))
        }
        checkMessageStackDepth(e2, allErrMsg(2, decorateToStringValue(prettifier, lists(2)) + " was not equal to " + decorateToStringValue(prettifier, Many(2, 3, 4)) + ", but " + decorateToStringValue(prettifier, lists(2)) + " contained at most one element of " + decorateToStringValue(prettifier, Seq(1, 2, 8)), thisLineNumber - 2, lists), fileName, thisLineNumber - 2)

        val e3 = intercept[TestFailedException] {
          all (hiLists) should (not be (Many("hi", "he")) and not contain atMostOneElementOf (Seq("ho", "hey", "howdy")))
        }
        checkMessageStackDepth(e3, allErrMsg(0, decorateToStringValue(prettifier, hiLists(0)) + " was equal to " + decorateToStringValue(prettifier, Many("hi", "he")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)

        val e4 = intercept[TestFailedException] {
          all (hiLists) should (not be (Many("hi", "ho")) and not contain atMostOneElementOf (Seq("hi", "hello")))
        }
        checkMessageStackDepth(e4, allErrMsg(0, decorateToStringValue(prettifier, hiLists(0)) + " was not equal to " + decorateToStringValue(prettifier, Many("hi", "ho")) + ", but " + decorateToStringValue(prettifier, hiLists(0)) + " contained at most one element of " + decorateToStringValue(prettifier, Seq("hi", "hello")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }

      it("should use the implicit Equality in scope") {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (not be (Many("HI", "HO")) and not contain atMostOneElementOf (Seq("HI", "HE")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be (Many("hi", "he")) and not contain atMostOneElementOf (Seq("hi", "he")))
        }
        ...
        val e2 = intercept[TestFailedException] {
          all (hiLists) should (not be (Many("hi", "ho")) and not contain atMostOneElementOf (Seq("HI", "HO")))
        }
      ...
      it("should do nothing when RHS contain duplicated value") {
        all (list1s) should (not be (Many(2, 3)) and not contain atMostOneElementOf (Seq(1, 2, 2, 3)))
      }
    }
  }
}
