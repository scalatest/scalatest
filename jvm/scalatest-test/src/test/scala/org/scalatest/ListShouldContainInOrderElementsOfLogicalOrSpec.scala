...
class ListShouldContainInOrderElementsOfLogicalOrSpec extends AnyFunSpec {
  ...
  describe("collection of Lists") {
    ...
    describe("when used with (be xx and contain inOrderElementsOf xx)") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        all (list1s) should (be (List(0, 1, 2, 2, 3)) or contain inOrderElementsOf Seq(1, 2, 3))
        all (list1s) should (be (List(8, 2, 3, 4)) or contain inOrderElementsOf Seq(1, 2, 3))
        all (list1s) should (be (List(0, 1, 2, 2, 3)) or contain inOrderElementsOf Seq(2, 3, 4))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (be (List(8, 2, 3, 4)) or contain inOrderElementsOf Seq(2, 3, 4))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(prettifier, list1s(0)) + " was not equal to " + decorateToStringValue(prettifier, List(8, 2, 3, 4)) + ", and " + decorateToStringValue(prettifier, list1s(0)) + " did not contain all elements of " + decorateToStringValue(prettifier, Seq(2, 3, 4)) + " in order", thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      it("should use the implicit Equality in scope") {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (be (List("he", "hi", "hello")) or contain inOrderElementsOf Seq("HI", "HELLO"))
        all (hiLists) should (be (List("ho", "hello")) or contain inOrderElementsOf Seq("HI", "HELLO"))
        all (hiLists) should (be (List("he", "hi", "hello")) or contain inOrderElementsOf Seq("HELLO", "HI"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be (List("ho", "hello")) or contain inOrderElementsOf Seq("HELLO", "HI"))
        }
      ...
      it("should do nothing when RHS contain duplicated value") {
        all (list1s) should (be (List(0, 1, 2, 2, 3)) or contain inOrderElementsOf Seq(1, 2, 2, 3))
      }
    }
    ...
    describe("when used with (not be xx and not contain inOrderElementsOf xx)") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        all (list1s) should (not be (List(2)) or not contain inOrderElementsOf (Seq(8, 3, 4)))
        all (list1s) should (not be (List(0, 1, 2, 2, 3)) or not contain inOrderElementsOf (Seq(8, 3, 4)))
        all (list1s) should (not be (List(2)) or not contain inOrderElementsOf (Seq(1, 2, 3)))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (not be (List(0, 1, 2, 2, 3)) or not contain inOrderElementsOf (Seq(1, 2, 3)))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(prettifier, list1s(0)) + " was equal to " + decorateToStringValue(prettifier, List(0, 1, 2, 2, 3)) + ", and " + decorateToStringValue(prettifier, list1s(0)) + " contained all elements of " + decorateToStringValue(prettifier, Seq(1, 2, 3)) + " in order", thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      it("should use the implicit Equality in scope") {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (not be (List("hello", "ho")) or not contain inOrderElementsOf (Seq("HELLO", "HO")))
        all (hiLists) should (not be (List("he", "hi", "hello")) or not contain inOrderElementsOf (Seq("HELLO", "HO")))
        all (hiLists) should (not be (List("hello", "ho")) or not contain inOrderElementsOf (Seq("HI", "HELLO")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be (List("he", "hi", "hello")) or not contain inOrderElementsOf (Seq("HI", "HELLO")))
        }
      ...
      it("should do nothing when RHS contain duplicated value") {
        all (list1s) should (not be (List(2)) or not contain inOrderElementsOf (Seq(1, 2, 2, 3)))
      }
    }
  }
}
