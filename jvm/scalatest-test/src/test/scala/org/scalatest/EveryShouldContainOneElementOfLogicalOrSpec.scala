...
class EveryShouldContainOneElementOfLogicalOrSpec extends AnyFunSpec {
  ...
  describe("every of Everys") {
    ...
    describe("when used with (be (..) and contain oneElementOf Seq(..))") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        ...
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(prettifier, One(1)) + " was not equal to " + decorateToStringValue(prettifier, One(2)) + ", and " + FailureMessages.didNotContainOneElementOf(prettifier, list1s(0), Seq(2, 3, 8)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }
    ...
    describe("when used with (not be (..) and not contain oneElementOf Seq(..))") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        ...
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(prettifier, One(1)) + " was equal to " + decorateToStringValue(prettifier, One(1)) + ", and " + FailureMessages.containedOneElementOf(prettifier, list1s(0), Seq(2, 3, 1)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }
