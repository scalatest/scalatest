...
class FreeSpecSpec extends funspec.AnyFunSpec with GivenWhenThen {
  ...
  describe("A path.FreeSpec") {
    ...
    describe("(when a nesting rule has been violated)") {
      ...
      it("should, if they call a - from within an in clause, result in a TestFailedException when running the test") {
        ...
        class MySpec extends PathAnyFreeSpec {
          //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new MySpec
          "should blow up" in {
    ...
    it("should return the test names in registration order from testNames") {
      ...
      class BFreeSpec extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new BFreeSpec
      }
      val b = new BFreeSpec
    ...
    describe("(with info calls)") {
      class InfoInsideTestSpec extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new InfoInsideTestSpec
      ...
      class InfoBeforeTestSpec extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new InfoBeforeTestSpec
        val msg = "hi there, dude"
        val testName = "test name"
        info(msg)
        testName in {}
    ...
    it("should throw NullArgumentException if a null test tag is provided") {
      ...
      class ExampleSpec1 extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new ExampleSpec1
        "hi" taggedAs(null) in {}
      }
      ...
      class ExampleSpec2 extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new ExampleSpec2
      ...
      class ExampleSpec3 extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new ExampleSpec3
      ...
      class ExampleSpec4 extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new ExampleSpec4
      ...
      class ExampleSpec5 extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new ExampleSpec5
      ...
      class ExampleSpec6 extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new ExampleSpec6
...
*/
  ...
  describe("when failure happens") {
    ...
    it("should generate NotAllowedException wrapping a TestFailedException when assert fails in scope") {
      class TestSpec extends PathAnyFreeSpec {
        //SCALATESTJS,NATIVE-ONLY override def newInstance: freespec.PathAnyFreeSpecLike = new TestSpec
        "a feature" - {
          val a = 1
          assert(a == 2)
        }
    ...
    it("should generate NotAllowedException wrapping a DuplicateTestNameException is thrown inside scope") {
      ...
      val e = intercept[NotAllowedException] {
        (new TestSpec).run(None, Args(SilentReporter))
      }
      assert("FreeSpecSpec.scala" == e.failedCodeFileName.get)
      assert(e.failedCodeLineNumber.get == thisLineNumber - 7)
      assert(e.cause.isDefined)
