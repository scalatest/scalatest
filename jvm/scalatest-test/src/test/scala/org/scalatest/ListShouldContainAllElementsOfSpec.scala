...
class ListShouldContainAllElementsOfSpec extends AnyFunSpec {
  ...
  describe("a List") {
    ...
    describe("when used with not contain allElementsOf Seq(..)") {
      ...
      it("should use an explicitly provided Equality") {
        (toList should not contain allElementsOf (Seq("happy", "birthday", "to", "you", "dear"))) (/* DOTTY-ONLY using */ decided by upperCaseStringEquality)
        intercept[TestFailedException] {
  ...
  describe("a col of Lists") {
    ...
    describe("when used with contain allElementsOf Seq(..)") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        ...
        e1.message should be (Some("'all' inspection failed, because: \n" +
          "  at index 2, " + decorateToStringValue(prettifier, lists(2)) + " did not contain all elements of " + decorateToStringValue(prettifier, Seq(1, 2, 3)) + " (ListShouldContainAllElementsOfSpec.scala:" + (thisLineNumber - 5) + ") \n" +
    ...
    describe("when used with not contain allElementsOf Seq(..)") {
      ...
      it("should do nothing if valid, else throw a TFE with an appropriate error message") {
        ...
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some("'all' inspection failed, because: \n" +
          "  at index 0, " + decorateToStringValue(prettifier, toLists(0)) + " contained all elements of " + decorateToStringValue(prettifier, Seq("you", "to")) +  " (ListShouldContainAllElementsOfSpec.scala:" + (thisLineNumber - 5) + ") \n" +
