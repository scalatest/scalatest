...
trait FixtureAnyFreeSpecLike extends org.scalatest.FixtureTestSuite with org.scalatest.FixtureTestRegistration with Informing with Notifying with Alerting with Documenting { thisSuite =>
  ...
  //DOTTY-ONLY   ${ source.Position.withPosition[Unit]('{(pos: source.Position) => engine.registerTest(testText, wrapTestFun(testFun), Resources.testCannotBeNestedInsideAnotherTest, "FixtureAnyFreeSpecLike.scala", "registerTest", 5, -2, None, None, Some(pos), None, testTags: _*)}) } 
  //DOTTY-ONLY }
  ...
  //DOTTY-ONLY   ${ source.Position.withPosition[Unit]('{(pos: source.Position) => engine.registerIgnoredTest(testText, wrapTestFun(testFun), Resources.testCannotBeNestedInsideAnotherTest, "FixtureAnyFreeSpecLike.scala", "registerIgnoredTest", 4, -3, None, Some(pos), testTags: _*)}) } 
  //DOTTY-ONLY }
