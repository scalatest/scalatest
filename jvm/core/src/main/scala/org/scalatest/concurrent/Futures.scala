...
trait Futures extends PatienceConfiguration {
  ...
  //DOTTY-ONLY   ${ Futures.whenReadyMacro('{future}, '{fun}, '{timeout.value}, '{interval.value}, '{pos}) }    
  ...
  //DOTTY-ONLY   ${ Futures.whenReadyMacro('{future}, '{fun}, '{timeout.value}, '{config.interval}, '{pos}) }  
  ...
  //DOTTY-ONLY   ${ Futures.whenReadyMacro('{future}, '{fun}, '{config.timeout}, '{interval.value}, '{pos}) }
  ...
  //DOTTY-ONLY   ${ Futures.whenReadyMacro('{future}, '{fun}, '{config.timeout}, '{config.interval}, '{pos}) }
...
object Futures extends Futures {
  ...
  //DOTTY-ONLY private[concurrent] def whenReadyMacro[T, U](future: Expr[FutureConcept[T]], fun: Expr[T => U], timeout: Expr[Span], interval: Expr[Span], pos: Expr[source.Position])(using quotes: Quotes, typeT: Type[T], typeU: Type[U]): Expr[U] = {
  //DOTTY-ONLY   source.Position.withCallerPosition[U](pos, '{(p: source.Position) => workaroundWhenReadyImpl(${future}, ${fun}, ${timeout}, ${interval}, p) })
  //DOTTY-ONLY }
