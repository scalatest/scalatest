    ...
    new Generator[Short] {
      ...
      case class NextRoseTree(value: Short) extends RoseTree[Short] {
        def shrinks(rndPassedToShrinks: Randomizer): (LazyList[RoseTree[Short]], Randomizer) = {
          def resLazyList(theValue: Short): LazyList[RoseTree[Short]] = {
            if (theValue == 0) LazyList.empty
            else {
              val half: Short = (theValue / 2).toShort
              if (half == 0) Rose(0.toShort) #:: LazyList.empty
    ...
    new Generator[Int] {
      ...
      case class NextRoseTree(value: Int) extends RoseTree[Int] {
        def shrinks(rndPassedToShrinks: Randomizer): (LazyList[RoseTree[Int]], Randomizer) = {
          def resLazyList(theValue: Int): LazyList[RoseTree[Int]] = {
            if (theValue == 0) LazyList.empty
            else {
              val half: Int = theValue / 2
              if (half == 0) Rose(0) #:: LazyList.empty
    ...
    new Generator[Long] {
      ...
      case class NextRoseTree(value: Long) extends RoseTree[Long] {
        def shrinks(rndPassedToShrinks: Randomizer): (LazyList[RoseTree[Long]], Randomizer) = {
          def resLazyList(theValue: Long): LazyList[RoseTree[Long]] = {
            if (theValue == 0) LazyList.empty
            else {
              val half: Long = (theValue / 2)
              if (half == 0) Rose(0L) #:: LazyList.empty
              else NextRoseTree(-half) #:: NextRoseTree(half) #:: resLazyList(half)
            }
          }
          (resLazyList(value), rndPassedToShrinks)
