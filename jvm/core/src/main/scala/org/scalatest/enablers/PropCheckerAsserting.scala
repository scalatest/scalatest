...
/**
 * A type class that provides the capability to perform property checks with generators.
 *
 * <p>
 * <code>PropCheckerAsserting</code> enables property-based testing by allowing you to check that
 * a property holds for all possible values generated by specified generators. This trait is designed
 * to support both synchronous and asynchronous property checking.
 * </p>
 *
 * <p>
 * The trait is parameterized with three type parameters:
 * </p>
 *
 * <ul>
 ...
 *     This is the final result that users receive after running a property check. It typically
 *     represents an assertion outcome (e.g., <code>Expectation</code>, <code>Assertion</code>,
 *     or <code>Future[Assertion]</code>) that indicates whether the property check passed or failed.</li>
 * <li><code>PropResult</code> - The type of result produced by evaluating the property function.
 ...
 *     For synchronous assertions, this is typically the same as the property function's return type
 *     (e.g., <code>Expectation</code> or <code>Assertion</code>). For asynchronous assertions
 *     (e.g., <code>Future[Assertion]</code>), this is the unwrapped type (e.g., <code>Assertion</code>),
 *     allowing the <code>discard</code> and <code>succeed</code> methods to operate on the actual result
 *     value after the Future completes.</li>
 * </ul>
 *
 * <p>
 * This trait provides the following key capabilities:
 * </p>
 *
 * <ul>
 * <li><code>discard</code> - Determines whether a property evaluation should be discarded (counted neither as success nor failure).</li>
 * <li><code>succeed</code> - Determines whether a property evaluation succeeded and optionally returns a cause if it failed.</li>
 * <li><code>check1</code> through <code>check6</code> - Methods to perform property checks with 1 to 6 generated values.</li>
 * </ul>
 *
 * <p>
 * Example usage:
 ...
 * // Implicit instance for synchronous assertions
 * implicit val assertingNatureOfAssertion: PropCheckerAsserting[Assertion, Assertion, Assertion] = ...
 *
 * // Implicit instance for asynchronous assertions
...
trait PropCheckerAsserting[T, CheckResult, PropResult] {
   ...
   *
   * <p>
   * A discarded evaluation is counted neither as a success nor as a failure. It is typically
   * used when the property cannot be meaningfully evaluated for a particular set of generated values,
   * for example, when preconditions are not met.
   * </p>
   *
   * @param result the result of evaluating the property function
   * @return <code>true</code> if the evaluation should be discarded, <code>false</code> otherwise
   */
  def discard(result: PropResult): Boolean

  /**
   * Determines whether a property evaluation succeeded and optionally returns a cause if it failed.
   *
   * <p>
   * This method checks the result of a property evaluation and returns a tuple where:
   * </p>
   * <ul>
   * <li>The first element is <code>true</code> if the evaluation succeeded, <code>false</code> otherwise</li>
   * <li>The second element is an <code>Option[Throwable]</code> containing the cause of failure if the evaluation failed</li>
   * </ul>
   *
   * @param result the result of evaluating the property function
   * @return a tuple where the first element indicates success and the second element optionally contains the failure cause
   */
...
abstract class ExpectationPropCheckerAsserting {
  ...
  // SKIP-DOTTY-START
  implicit def assertingNatureOfExpectation(implicit prettifier: Prettifier): PropCheckerAsserting[Expectation, Expectation, Expectation] = {
  // SKIP-DOTTY-END
  //DOTTY-ONLY def assertingNatureOfExpectation(using prettifier: Prettifier): PropCheckerAsserting[Expectation, Expectation, Expectation] = {
    new PropCheckerAssertingImpl[Expectation, Expectation] {
  ...
  //DOTTY-ONLY given given_assertingNatureOfExpectation(using prettifier: Prettifier): PropCheckerAsserting[Expectation, Expectation, Expectation] = assertingNatureOfExpectation(using prettifier)
}
...
object PropCheckerAsserting extends ExpectationPropCheckerAsserting {
  ...
  // SKIP-DOTTY-START
  implicit def assertingNatureOfAssertion: PropCheckerAsserting[Assertion, Assertion, Assertion] = {
  // SKIP-DOTTY-END
  //DOTTY-ONLY def assertingNatureOfAssertion: PropCheckerAsserting[Assertion, Assertion, Assertion] = {
    new PropCheckerAssertingImpl[Assertion, Assertion] {
  ...
  //DOTTY-ONLY given given_assertingNatureOfAssertion: PropCheckerAsserting[Assertion, Assertion, Assertion] = assertingNatureOfAssertion

  // SKIP-DOTTY-START
  implicit def assertingNatureOfFutureAssertion(implicit exeCtx: scala.concurrent.ExecutionContext): PropCheckerAsserting[Future[Assertion], Future[Assertion], Assertion] = {
  // SKIP-DOTTY-END
  //DOTTY-ONLY def assertingNatureOfFutureAssertion(using exeCtx: scala.concurrent.ExecutionContext): PropCheckerAsserting[Future[Assertion], Future[Assertion], Assertion] = {
  ...
  //DOTTY-ONLY given given_assertingNatureOfFutureAssertion(using exeCtx: scala.concurrent.ExecutionContext): PropCheckerAsserting[Future[Assertion], Future[Assertion], Assertion] = assertingNatureOfFutureAssertion(using exeCtx)

  // SKIP-DOTTY-START
  implicit def assertingNatureOfFutureAssertionForGeneratedCode(implicit exeCtx: scala.concurrent.ExecutionContext): PropCheckerAsserting[Future[Assertion], Future[Assertion], Future[Assertion]] = {
    val underlying = assertingNatureOfFutureAssertion(exeCtx)
    new PropCheckerAsserting[Future[Assertion], Future[Assertion], Future[Assertion]] {
      def discard(result: Future[Assertion]): Boolean = {
        // Map Future[Assertion] to Assertion by checking if it's already completed
        result.value match {
          case Some(Success(assertion)) => underlying.discard(assertion)
          case Some(Failure(_)) => true  // Failed futures should be discarded
          case None => false  // Not yet completed, don't discard yet
        }
      }
      def succeed(result: Future[Assertion]): (Boolean, Option[Throwable]) = {
        result.value match {
          case Some(Success(assertion)) => underlying.succeed(assertion)
          case Some(Failure(ex)) => (false, Some(ex))
          case None => (true, None)  // Not yet completed, treat as success for now
        }
      }
      def check1[A](fun: (A) => Future[Assertion],
                   genA: org.scalatest.prop.Generator[A],
                   prms: Configuration.Parameter,
                   prettifier: Prettifier,
                   pos: source.Position,
                   names: List[String],
                   argNames: Option[List[String]] = None): Future[Assertion] = {
        underlying.check1(fun, genA, prms, prettifier, pos, names, argNames)
      }
      def check2[A, B](fun: (A, B) => Future[Assertion],
                       genA: org.scalatest.prop.Generator[A],
                       genB: org.scalatest.prop.Generator[B],
                       prms: Configuration.Parameter,
                       prettifier: Prettifier,
                       ...
                       names: List[String],
                       argNames: Option[List[String]] = None): Future[Assertion] = {
        underlying.check2(fun, genA, genB, prms, prettifier, pos, names, argNames)
      }
      def check3[A, B, C](fun: (A, B, C) => Future[Assertion],
                          genA: org.scalatest.prop.Generator[A],
                          genB: org.scalatest.prop.Generator[B],
                          genC: org.scalatest.prop.Generator[C],
                          prms: Configuration.Parameter,
                          ...
                          names: List[String],
                          argNames: Option[List[String]] = None): Future[Assertion] = {
        underlying.check3(fun, genA, genB, genC, prms, prettifier, pos, names, argNames)
      }
      def check4[A, B, C, D](fun: (A, B, C, D) => Future[Assertion],
                             genA: org.scalatest.prop.Generator[A],
                             genB: org.scalatest.prop.Generator[B],
                             genC: org.scalatest.prop.Generator[C],
                             genD: org.scalatest.prop.Generator[D],
                             ...
                             names: List[String],
                             argNames: Option[List[String]] = None): Future[Assertion] = {
        underlying.check4(fun, genA, genB, genC, genD, prms, prettifier, pos, names, argNames)
      }
      def check5[A, B, C, D, E](fun: (A, B, C, D, E) => Future[Assertion],
                                genA: org.scalatest.prop.Generator[A],
                                genB: org.scalatest.prop.Generator[B],
                                genC: org.scalatest.prop.Generator[C],
                                genD: org.scalatest.prop.Generator[D],
                                ...
                                names: List[String],
                                argNames: Option[List[String]] = None): Future[Assertion] = {
        underlying.check5(fun, genA, genB, genC, genD, genE, prms, prettifier, pos, names, argNames)
      }
      def check6[A, B, C, D, E, F](fun: (A, B, C, D, E, F) => Future[Assertion],
                                   genA: org.scalatest.prop.Generator[A],
                                   genB: org.scalatest.prop.Generator[B],
                                   genC: org.scalatest.prop.Generator[C],
                                   genD: org.scalatest.prop.Generator[D],
                                   ...
                                   names: List[String],
                                   argNames: Option[List[String]] = None): Future[Assertion] = {
        underlying.check6(fun, genA, genB, genC, genD, genE, genF, prms, prettifier, pos, names, argNames)
      }
    }
      }
      // SKIP-DOTTY-END
    
      //DOTTY-ONLY given given_assertingNatureOfFutureAssertionForGeneratedCode(using exeCtx: scala.concurrent.ExecutionContext): PropCheckerAsserting[Future[Assertion], Future[Assertion], Future[Assertion]] = {
      //DOTTY-ONLY   val underlying = assertingNatureOfFutureAssertion(using exeCtx)
      //DOTTY-ONLY   new PropCheckerAsserting[Future[Assertion], Future[Assertion], Future[Assertion]] {
      //DOTTY-ONLY     def discard(result: Future[Assertion]): Boolean = {
      //DOTTY-ONLY       result.value match {
      //DOTTY-ONLY         case Some(Success(assertion)) => underlying.discard(assertion)
      //DOTTY-ONLY         case Some(Failure(_)) => true
      //DOTTY-ONLY         case None => false
      //DOTTY-ONLY       }
      //DOTTY-ONLY     }
      //DOTTY-ONLY     def succeed(result: Future[Assertion]): (Boolean, Option[Throwable]) = {
      //DOTTY-ONLY       result.value match {
      //DOTTY-ONLY         case Some(Success(assertion)) => underlying.succeed(assertion)
      //DOTTY-ONLY         case Some(Failure(ex)) => (false, Some(ex))
      //DOTTY-ONLY         case None => (true, None)
      //DOTTY-ONLY       }
      //DOTTY-ONLY     }
      //DOTTY-ONLY     def check1[A](fun: (A) => Future[Assertion],
      //DOTTY-ONLY                          genA: org.scalatest.prop.Generator[A],
      //DOTTY-ONLY                          prms: Configuration.Parameter,
      //DOTTY-ONLY                          prettifier: Prettifier,
      //DOTTY-ONLY                          pos: source.Position,
      //DOTTY-ONLY                          names: List[String],
      //DOTTY-ONLY                          argNames: Option[List[String]] = None): Future[Assertion] =
      //DOTTY-ONLY       underlying.check1(fun, genA, prms, prettifier, pos, names, argNames)
      //DOTTY-ONLY     def check2[A, B](fun: (A, B) => Future[Assertion],
      //DOTTY-ONLY                           genA: org.scalatest.prop.Generator[A],
      //DOTTY-ONLY                           genB: org.scalatest.prop.Generator[B],
      //DOTTY-ONLY                           prms: Configuration.Parameter,
      //DOTTY-ONLY                           prettifier: Prettifier,
      //DOTTY-ONLY                           pos: source.Position,
      //DOTTY-ONLY                           names: List[String],
      //DOTTY-ONLY                           argNames: Option[List[String]] = None): Future[Assertion] =
      //DOTTY-ONLY       underlying.check2(fun, genA, genB, prms, prettifier, pos, names, argNames)
      //DOTTY-ONLY     def check3[A, B, C](fun: (A, B, C) => Future[Assertion],
      //DOTTY-ONLY                               genA: org.scalatest.prop.Generator[A],
      //DOTTY-ONLY                               genB: org.scalatest.prop.Generator[B],
      //DOTTY-ONLY                               genC: org.scalatest.prop.Generator[C],
      //DOTTY-ONLY                               prms: Configuration.Parameter,
      //DOTTY-ONLY                               prettifier: Prettifier,
      //DOTTY-ONLY                               pos: source.Position,
      //DOTTY-ONLY                               names: List[String],
      //DOTTY-ONLY                               argNames: Option[List[String]] = None): Future[Assertion] =
      //DOTTY-ONLY       underlying.check3(fun, genA, genB, genC, prms, prettifier, pos, names, argNames)
      //DOTTY-ONLY     def check4[A, B, C, D](fun: (A, B, C, D) => Future[Assertion],
      //DOTTY-ONLY                              genA: org.scalatest.prop.Generator[A],
      //DOTTY-ONLY                              genB: org.scalatest.prop.Generator[B],
      //DOTTY-ONLY                              genC: org.scalatest.prop.Generator[C],
      //DOTTY-ONLY                              genD: org.scalatest.prop.Generator[D],
      //DOTTY-ONLY                              prms: Configuration.Parameter,
      //DOTTY-ONLY                              prettifier: Prettifier,
      //DOTTY-ONLY                              pos: source.Position,
      //DOTTY-ONLY                              names: List[String],
      //DOTTY-ONLY                              argNames: Option[List[String]] = None): Future[Assertion] =
      //DOTTY-ONLY       underlying.check4(fun, genA, genB, genC, genD, prms, prettifier, pos, names, argNames)
      //DOTTY-ONLY     def check5[A, B, C, D, E](fun: (A, B, C, D, E) => Future[Assertion],
      //DOTTY-ONLY                               genA: org.scalatest.prop.Generator[A],
      //DOTTY-ONLY                               genB: org.scalatest.prop.Generator[B],
      //DOTTY-ONLY                               genC: org.scalatest.prop.Generator[C],
      //DOTTY-ONLY                               genD: org.scalatest.prop.Generator[D],
      //DOTTY-ONLY                               genE: org.scalatest.prop.Generator[E],
      //DOTTY-ONLY                               prms: Configuration.Parameter,
      //DOTTY-ONLY                               prettifier: Prettifier,
      //DOTTY-ONLY                               pos: source.Position,
      //DOTTY-ONLY                               names: List[String],
      //DOTTY-ONLY                               argNames: Option[List[String]] = None): Future[Assertion] =
      //DOTTY-ONLY       underlying.check5(fun, genA, genB, genC, genD, genE, prms, prettifier, pos, names, argNames)
      //DOTTY-ONLY     def check6[A, B, C, D, E, F](fun: (A, B, C, D, E, F) => Future[Assertion],
      //DOTTY-ONLY                                genA: org.scalatest.prop.Generator[A],
      //DOTTY-ONLY                                genB: org.scalatest.prop.Generator[B],
      //DOTTY-ONLY                                genC: org.scalatest.prop.Generator[C],
      //DOTTY-ONLY                                genD: org.scalatest.prop.Generator[D],
      //DOTTY-ONLY                                genE: org.scalatest.prop.Generator[E],
      //DOTTY-ONLY                                genF: org.scalatest.prop.Generator[F],
      //DOTTY-ONLY                                prms: Configuration.Parameter,
      //DOTTY-ONLY                                prettifier: Prettifier,
      //DOTTY-ONLY                                pos: source.Position,
      //DOTTY-ONLY                                names: List[String],
      //DOTTY-ONLY                                argNames: Option[List[String]] = None): Future[Assertion] =
      //DOTTY-ONLY       underlying.check6(fun, genA, genB, genC, genD, genE, genF, prms, prettifier, pos, names, argNames)
      //DOTTY-ONLY   }
      //DOTTY-ONLY }
    
      private[enablers] def argsAndLabels(result: PropertyCheckResult): (List[PropertyArgument], List[String]) = {    val (args: List[PropertyArgument], labels: List[String]) =
      result match {
