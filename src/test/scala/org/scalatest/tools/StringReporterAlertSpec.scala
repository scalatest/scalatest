...
class StringReporterAlertSpec extends UnitSpec {
  ...
  val NoticeText = "a suite name: a test name: some notice"
  ...
  object `A StringReporter` {
    object `when presentUnformatted is false but no formatter is supplied` {
      ...
      def `should display NoticeProvided messages in green and unformatted` {
      
        fragmentsForEvent(
          event = NoticeProvided(new Ordinal(0), "some notice", Some(NameInfo("a suite name", "a suite Id", Some("a suite class"), Some("a test name")))),
          presentUnformatted = false,
          presentAllDurations = false,
          presentShortStackTraces = false,
          presentFullStackTraces = false,
          ...
          presentReminderWithoutCanceledTests = false,
          reminderEvents = Seq.empty
        ) shouldEqual Vector(Fragment(Resources("noticeProvided", NoticeText), AnsiGreen))
      }
    }
        ...
        fragmentsForEvent(
          event =
            NoticeProvided(
              ...
              Some(NameInfo("a suite name", "a suite Id", Some("a suite class"), Some("a test name"))),
              None,
              Some(IndentedText(Resources("noticeFormattedText", "some notice"), "some notice", 0))
            ),
          presentUnformatted = true,
          presentAllDurations = false,
          presentShortStackTraces = false,
    ...
    object `when a formatter is supplied and presentUnformatted is false` {
      ...
      def `should display NoticeProvided messages formatted in green` {
        ...
        fragmentsForEvent(
          ...
          presentReminderWithoutCanceledTests = false,
          reminderEvents = Seq.empty
        ) shouldEqual Vector(Fragment(Resources("noticeFormattedText", "some notice "), AnsiGreen))
      }
    }
  }
}
