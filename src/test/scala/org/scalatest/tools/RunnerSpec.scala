...
class RunnerSpec extends Spec with PrivateMethodTester {
  ...
  def `parseSuiteArgs should work correctly` {
    ...
    assert(tests9(0).isGlob === false)

    //
    // A list starting with a -z should produce a corresponding test spec.
    //
    val (suites11, tests11) = Runner.parseSuiteArgs(List("-z", "glob1"))
    assert(suites11.isEmpty)
    assert(tests11.length === 1)
    assert(tests11(0).spec === "glob1")
    assert(tests11(0).isGlob === true)
    ...
    assert(tests12(0).isGlob === false)
    assert(tests12(1).spec === "glob1")
    assert(tests12(1).isGlob === true)
    assert(tests12(2).spec === "test2")
    assert(tests12(2).isGlob === false)
    assert(tests12(3).spec === "test3")
    assert(tests12(3).isGlob === false)
    assert(tests12(4).spec === "glob2")
    assert(tests12(4).isGlob === true)
  }
