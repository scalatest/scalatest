...
class ShouldBeDoubleEqualsNullSpec extends Spec with Checkers with ReturnsNormallyThrowsAssertion {

  val nullMap: Map[Int, String] = null
  val map = Map(1 -> "one", 2 -> "two")

  object `the be_== (null) syntax` {
    ...
    def `should work in its basic for with or without not` {
      ...
      map should (not be_== (null))
      map should (not (be (null)))
      nullMap should be_== (null)
      nullMap should (be (null))
      // null should be_== (null) this doesn't compile. can't do implicits on the Null type, which is fine
      // null should (be (null))
    ...
    def `should throw a NullPointerException if they try to short circuit with a null check first` {
      ...
      intercept[NullPointerException] {
        nullMap should (not be_== (null) and contain key (7))
      }
      intercept[NullPointerException] {
    ...
    def `should compile and run when used with and (with or without not)` {
      ...
      val caught1 = intercept[TestFailedException] {
        Map(1 -> "one") should (contain key (7) and not be_== (null))
      }
      ...
      Map(1 -> "one") should (contain key (1) and not be_== (null))
      Map(1 -> "one") should (contain key (1) and not (be (null)))
      Map(1 -> "one") should (contain key (1) and (not (be (null))))

      val caught4 = intercept[TestFailedException] {
        Map(1 -> "one") should (contain key (1) and be_== (null))
      }
      ...
      val caught6 = intercept[TestFailedException] {
        Map(1 -> "one") should (be (null) and not be_== (null))
      }
      ...
      nullMap should (be (null) and be_== (null))
      nullMap should (be (null) and (be (null)))

      val caught9 = intercept[TestFailedException] {
        nullMap should (be (null) and not be_== (null))
      }
    ...
    def `should compile and run when used with or (with or without not)` {
      ...
      Map(1 -> "one") should (contain key (7) or not be_== (null))
      Map(1 -> "one") should (contain key (7) or not (be (null)))
      Map(1 -> "one") should (contain key (7) or (not (be (null))))

      Map(1 -> "one") should (contain key (1) or not be_== (null))
      Map(1 -> "one") should (contain key (1) or not (be (null)))
      Map(1 -> "one") should (contain key (1) or (not (be (null))))

      val caught4 = intercept[TestFailedException] {
        Map(1 -> "one") should (contain key (7) or be_== (null))
      }
      ...
      val caught6 = intercept[TestFailedException] {
        Map(1 -> "one") should (be (null) or be_== (null))
      }
      ...
      nullMap should (be (null) or be_== (null))
      nullMap should (be (null) or (be (null)))
      nullMap should (be (null) or not be_== (null))
      nullMap should (be (null) or not (be (null)))
      nullMap should (be (null) or (not (be (null))))
      nullMap should (not be_== (null) or be_== (null))
      nullMap should (not (be (null)) or be_== (null))
    }
  }
