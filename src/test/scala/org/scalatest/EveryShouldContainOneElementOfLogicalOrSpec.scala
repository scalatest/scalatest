...
class EveryShouldContainOneElementOfLogicalOrSpec extends Spec {
  ...
  object `an Every` {
    ...
    object `when used with (equal (..) and contain oneElementOf Seq(..))` {
    ...
    object `when used with (be (..) and contain oneElementOf Seq(..))` {
    ...
    object `when used with (contain oneElementOf Seq(..) and be (..))` {
    ...
    object `when used with (not contain oneElementOf Seq(..) and not contain oneElementOf Seq(..))` {
    ...
    object `when used with (not equal (..) and not contain oneElementOf Seq(..))` {
    ...
    object `when used with (not be (..) and not contain oneElementOf Seq(..))` {
  ...
  object `every of Everys` {
    ...
    object `when used with (contain oneElementOf Seq(..) and contain oneElementOf Seq(..))` {
    ...
    object `when used with (be (..) and contain oneElementOf Seq(..))` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (be_== (One(1)) or contain oneElementOf Seq(1, 3, 4))
        all (list1s) should (be_== (One(2)) or contain oneElementOf Seq(1, 3, 4))
        all (list1s) should (be_== (One(1)) or contain oneElementOf Seq(2, 3, 4))
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (be_== (One("hi")) or contain oneElementOf Seq("HI", "HE"))
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (be_== (One("hi")) or contain oneElementOf Seq("HI", "HE"))) (decided by upperCaseStringEquality)
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (be_== (One(1)) or contain oneElementOf Seq(3, 2, 2, 1))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("oneElementOfDuplicate")))
    ...
    object `when used with (not be (..) and not contain oneElementOf Seq(..))` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (not be_== (One(2)) or not contain oneElementOf (Seq(8, 3, 4)))
        all (list1s) should (not be_== (One(1)) or not contain oneElementOf (Seq(8, 3, 4)))
        all (list1s) should (not be_== (One(2)) or not contain oneElementOf (Seq(8, 3, 1)))
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (not be_== (One("ho")) or not contain oneElementOf (Seq("hi", "he")))
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (not be_== (One("ho")) or not contain oneElementOf (Seq("hi", "he")))) (decided by upperCaseStringEquality)
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (not be_== (One(2)) or not contain oneElementOf (Seq(3, 2, 2, 1)))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("oneElementOfDuplicate")))
