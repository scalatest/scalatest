...
class EveryShouldContainAllElementsOfLogicalAndSpec extends Spec {
  ...
  object `col of Lists` {
    ...
    object `when used with (be (..) and contain allElementsOf Seq(..))` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (be_== (Many(3, 2, 1, 0)) and contain allElementsOf Seq(1, 3, 2))
        atLeast (2, lists) should (be_== (Many(3, 2, 1, 0)) and contain allElementsOf Seq(1, 3, 2))
        atMost (2, lists) should (be_== (Many(3, 2, 1, 0)) and contain allElementsOf Seq(2, 3, 1))
        no (lists) should (be_== (Many(3, 6, 9)) and contain allElementsOf Seq(3, 4, 5))

        val e1 = intercept[TestFailedException] {
          all (lists) should (be_== (Many(3, 2, 1, 0)) and contain allElementsOf Seq(1, 3, 2))
        }
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (be_== (Many("howdy", "hi", "hello")) and contain allElementsOf Seq("HELLO", "HI"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be_== (Many("HI", "HELLO")) and contain allElementsOf Seq("HELLO", "HI"))
        }
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (be_== (Many("howdy", "hi", "hello")) and contain allElementsOf Seq("HELLO", "HI"))) (decided by upperCaseStringEquality)
        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (be_== (Many("HI", "HELLO")) and contain allElementsOf Seq("HELLO", "HI"))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(Many("howdy", "hi", "hello")) + " was not equal to " + decorateToStringValue(Many("HI", "HELLO")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
    ...
    object `when used with (not be (..) and not contain allElementsOf Seq(..))` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (not be_== (One(2)) and not contain allElementsOf (Seq(8, 3, 4)))
        atLeast (2, lists) should (not be_== (One(3)) and not contain allElementsOf (Seq(8, 3, 4)))
        atMost (2, lists) should (not be_== (Many(4, 3, 2)) and not contain allElementsOf (Seq(3, 4, 2)))
        no (list1s) should (not be_== (Many(3, 2, 1)) and not contain allElementsOf (Seq(1, 2, 3)))

        val e1 = intercept[TestFailedException] {
          all (lists) should (not be_== (Many(8, 4, 3, 2)) and not contain allElementsOf (Seq(8, 3, 4)))
        }
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (not be_== (One("ho")) and not contain allElementsOf (Seq("HO", "HELLO")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (Many("howdy", "hi", "hello")) and not contain allElementsOf (Seq("HELLO", "HI")))
        }
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (not be_== (One("ho")) and not contain allElementsOf (Seq("HO", "HELLO")))) (decided by upperCaseStringEquality)
        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (not be_== (Many("howdy", "hi", "hello")) and not contain allElementsOf (Seq("HELLO", "HI")))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(Many("howdy", "hi", "hello")) + " was equal to " + decorateToStringValue(Many("howdy", "hi", "hello")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (not be_== (One(2)) and not contain allElementsOf (Seq(3, 2, 2, 1)))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("allElementsOfDuplicate")))
