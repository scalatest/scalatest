...
class EveryShouldContainAtLeastOneElementOfLogicalOrSpec extends FreeSpec {
  ...
  val fileName: String = "EveryShouldContainAtLeastOneElementOfLogicalOrSpec.scala"
  ...
  "every of Everys" - {
    ...
    "when used with (contain atLeastOneElementOf (..) and contain atLeastOneElementOf (..)) syntax" - {
      ...
      "should use the implicit Equality in scope" in {
        ...
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " did not contain at least one element of " + decorateToStringValue(List("hi", "he")) + ", and " + decorateToStringValue(hiLists(0)) + " did not contain at least one element of " + decorateToStringValue(Seq("hi", "he")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
    ...
    "when used with (be (...) and contain atLeastOneElementOf (...)) syntax" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (be_== (One(1)) or contain atLeastOneElementOf Seq(1, 3, 4))
        all (list1s) should (be_== (One(2)) or contain atLeastOneElementOf Seq(1, 3, 4))
        all (list1s) should (be_== (One(1)) or contain atLeastOneElementOf Seq(2, 3, 4))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (be_== (One(2)) or contain atLeastOneElementOf Seq(2, 3, 8))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(One(1)) + " was not equal to " + decorateToStringValue(One(2)) + ", and " + decorateToStringValue(list1s(0)) + " did not contain at least one element of " + decorateToStringValue(Seq(2, 3, 8)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      "should use the implicit Equality in scope" in {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (be_== (One("hi")) or contain atLeastOneElementOf Seq("HI", "HE"))
        all (hiLists) should (be_== (One("ho")) or contain atLeastOneElementOf Seq("HI", "HE"))
        all (hiLists) should (be_== (One("hi")) or contain atLeastOneElementOf Seq("hi", "he"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be_== (One("ho")) or contain atLeastOneElementOf Seq("hi", "he"))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(One("hi")) + " was not equal to " + decorateToStringValue(One("ho")) + ", and " + decorateToStringValue(hiLists(0)) + " did not contain at least one element of " + decorateToStringValue(Seq("hi", "he")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }

      "should use an explicitly provided Equality" in {
        (all (hiLists) should (be_== (One("hi")) or contain atLeastOneElementOf Seq("HI", "HE"))) (decided by upperCaseStringEquality)
        (all (hiLists) should (be_== (One("ho")) or contain atLeastOneElementOf Seq("HI", "HE"))) (decided by upperCaseStringEquality)
        (all (hiLists) should (be_== (One("hi")) or contain atLeastOneElementOf Seq("hi", "he"))) (decided by upperCaseStringEquality)
      ...
      "should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value" in {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (be_== (One(1)) or contain atLeastOneElementOf Seq(3, 2, 2, 1))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("atLeastOneElementOfDuplicate")))
    ...
    "when used with (not contain atLeastOneElementOf (..) and not contain atLeastOneElementOf (..))" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        ...
        checkMessageStackDepth(e1, allErrMsg(2, decorateToStringValue(lists(2)) + " contained at least one element of " + decorateToStringValue(Seq(2, 6, 8)) + ", and " + decorateToStringValue(lists(2)) + " contained at least one element of " + decorateToStringValue(Seq(2, 3, 4)), thisLineNumber - 2, lists), fileName, thisLineNumber - 2)
      }
    ...
    "when used with (not be (...) and not contain atLeastOneElementOf (...))" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (not be_== (One(2)) or not contain atLeastOneElementOf (Seq(8, 3, 4)))
        all (list1s) should (not be_== (One(1)) or not contain atLeastOneElementOf (Seq(8, 3, 4)))
        all (list1s) should (not be_== (One(2)) or not contain atLeastOneElementOf (Seq(8, 3, 1)))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (not be_== (One(1)) or not contain atLeastOneElementOf (Seq(2, 3, 1)))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(One(1)) + " was equal to " + decorateToStringValue(One(1)) + ", and " + decorateToStringValue(One(1)) + " contained at least one element of " + decorateToStringValue(Seq(2, 3, 1)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      "should use the implicit Equality in scope" in {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (not be_== (One("ho")) or not contain atLeastOneElementOf (Seq("hi", "he")))
        all (hiLists) should (not be_== (One("hi")) or not contain atLeastOneElementOf (Seq("hi", "he")))
        all (hiLists) should (not be_== (One("ho")) or not contain atLeastOneElementOf (Seq("HI", "HE")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (One("hi")) or not contain atLeastOneElementOf (Seq("HI", "HE")))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(One("hi")) + " was equal to " + decorateToStringValue(One("hi")) + ", and " + decorateToStringValue(hiLists(0)) + " contained at least one element of " + decorateToStringValue(Seq("HI", "HE")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }

      "should use an explicitly provided Equality" in {
        (all (hiLists) should (not be_== (One("ho")) or not contain atLeastOneElementOf (Seq("hi", "he")))) (decided by upperCaseStringEquality)
        (all (hiLists) should (not be_== (One("hi")) or not contain atLeastOneElementOf (Seq("hi", "he")))) (decided by upperCaseStringEquality)
        (all (hiLists) should (not be_== (One("ho")) or not contain atLeastOneElementOf (Seq("HI", "HE")))) (decided by upperCaseStringEquality)
      ...
      "should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value" in {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (not be_== (One(2)) or not contain atLeastOneElementOf (Seq(3, 2, 2, 1)))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("atLeastOneElementOfDuplicate")))
