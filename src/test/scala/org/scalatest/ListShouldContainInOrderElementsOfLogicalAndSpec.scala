...
class ListShouldContainInOrderElementsOfLogicalAndSpec extends Spec {
  ...
  object `col of Lists` {
    ...
    object `when used with (be xx and contain inOrder xx)` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (be_== (List(0, 1, 2, 2, 3)) and contain inOrderElementsOf Seq(1, 2, 3))
        atLeast (2, lists) should (be_== (List(0, 1, 2, 3, 3)) and contain inOrderElementsOf Seq(1, 2, 3))
        atMost (2, lists) should (be_== (List(3, 2, 1)) and contain inOrderElementsOf Seq(1, 2, 3))
        no (lists) should (be_== (List(3, 6, 9)) and contain inOrderElementsOf Seq(3, 4, 5))
        no (nils) should (be_== (List(1, 6, 8)) and contain inOrderElementsOf Seq(1, 3, 4))
        no (listsNil) should (be_== (List(2, 6, 8)) and contain inOrderElementsOf Seq(3, 4, 5))

        val e1 = intercept[TestFailedException] {
          all (lists) should (be_== (List(0, 1, 2, 3, 3)) and contain inOrderElementsOf Seq(1, 2, 3))
        }
        ...
        val e2 = intercept[TestFailedException] {
          all (list1s) should (be_== (List(0, 1, 2, 2, 3)) and contain inOrderElementsOf Seq(2, 3, 8))
        }
        ...
        val e3 = intercept[TestFailedException] {
          all (nils) should (be_== (List("hey")) and contain inOrderElementsOf Seq("hi", "hello"))
        }
        ...
        val e4 = intercept[TestFailedException] {
          all (hiLists) should (be_== (List("he", "hi", "hello")) and contain inOrderElementsOf Seq("hello", "hi"))
        }
        ...
        val e5 = intercept[TestFailedException] {
          all (listsNil) should (be_== (List(0, 1, 2, 3)) and contain inOrderElementsOf Seq(1, 2, 3))
        }
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (be_== (List("he", "hi", "hello")) and contain inOrderElementsOf Seq("HI", "HELLO"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be_== (List("HI", "HELLO")) and contain inOrderElementsOf Seq("HI", "HELLO"))
        }
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (be_== (List("he", "hi", "hello")) and contain inOrderElementsOf Seq("HI", "HELLO"))) (decided by upperCaseStringEquality)
        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (be_== (List("HI", "HELLO")) and contain inOrderElementsOf Seq("HI", "HELLO"))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was not equal to " + decorateToStringValue(List("HI", "HELLO")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)

        val e2 = intercept[TestFailedException] {
          (all (hiLists) should (be_== (List("he", "hi", "hello")) and contain inOrderElementsOf Seq("HELLO", "HI"))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e2, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was equal to " + decorateToStringValue(List("he", "hi", "hello")) + ", but " + decorateToStringValue(hiLists(0)) + " did not contain all elements of " + decorateToStringValue(Seq("HELLO", "HI")) + " in order", thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (be_== (List(0, 1, 2, 2, 3)) and contain inOrderElementsOf Seq(1, 2, 2, 3))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("inOrderElementsOfDuplicate")))
    ...
    object `when used with (not be xx and not contain inOrder xx)` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (not be_== (List(2)) and not contain inOrderElementsOf (Seq(8, 3, 4)))
        atLeast (2, lists) should (not be_== (List(3)) and not contain inOrderElementsOf (Seq(8, 3, 4)))
        atMost (2, lists) should (not be_== (List(8, 2, 3, 4)) and not contain inOrderElementsOf (Seq(2, 3, 4)))
        no (list1s) should (not be_== (List(0, 1, 2, 2, 3)) and not contain inOrderElementsOf (Seq(1, 2, 3)))

        val e1 = intercept[TestFailedException] {
          all (lists) should (not be_== (List(8, 2, 3, 4)) and not contain inOrderElementsOf (Seq(8, 3, 4)))
        }
        ...
        val e2 = intercept[TestFailedException] {
          all (lists) should (not be_== (List(3)) and not contain inOrderElementsOf (Seq(2, 3, 4)))
        }
        ...
        val e3 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (List("he", "hi", "hello")) and not contain inOrderElementsOf (Seq("ho", "hey", "howdy")))
        }
        ...
        val e4 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (List("ho")) and not contain inOrderElementsOf (Seq("hi", "hello")))
        }
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (not be_== (List("ho")) and not contain inOrderElementsOf (Seq("HO", "HELLO")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (List("he", "hi", "hello")) and not contain inOrderElementsOf (Seq("HELLO", "HI")))
        }
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (not be_== (List("ho")) and not contain inOrderElementsOf (Seq("HO", "HELLO")))) (decided by upperCaseStringEquality)
        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (not be_== (List("he", "hi", "hello")) and not contain inOrderElementsOf (Seq("HELLO", "HI")))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was equal to " + decorateToStringValue(List("he", "hi", "hello")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)

        val e2 = intercept[TestFailedException] {
          (all (hiLists) should (not be_== (List("ho")) and not contain inOrderElementsOf (Seq("HI", "HELLO")))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e2, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was not equal to " + decorateToStringValue(List("ho")) + ", but " + decorateToStringValue(hiLists(0)) + " contained all elements of " + decorateToStringValue(Seq("HI", "HELLO")) + " in order", thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (not be_== (List(2)) and not contain inOrderElementsOf (Seq(1, 2, 2, 3)))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("inOrderElementsOfDuplicate")))
