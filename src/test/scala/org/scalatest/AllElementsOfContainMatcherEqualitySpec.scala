...
import org.scalactic.{Prettifier, Equality, Entry}
import org.scalactic.Explicitly._
import collection.GenTraversable
import SharedHelpers._
import Matchers._

class AllElementsOfContainMatcherEqualitySpec extends Spec {
  ...
  object `allElementsOf ` {
    ...
    def `should throw TestFailedException with correct stack depth and message when 'should not contain custom matcher' failed with custom implicit equality in scope` {
      ...
      val left5 = Map(1 -> "one ", 2 -> "two", 3 -> "three ")
      val e5 = intercept[exceptions.TestFailedException] {
        left5 should not contain allElementsOf (Seq(1 -> "one", 2 -> "two ", 3 -> "three"))
      }
    ...
    def `should take passed in custom explicit equality when 'should not contain' is used` {
      ...
      (List(1, 2, 3) should not contain allElementsOf (Seq(1, 2, 3))) (equality)
      (Set(1, 2, 3) should not contain allElementsOf (Seq(1, 2, 3))) (equality)
      (Array(1, 2, 3) should not contain allElementsOf (Seq(1, 2, 3))) (equality)
    ...
    def `should throw TestFailedException with correct stack depth and message when 'should contain custom matcher' failed with custom explicit equality` {
      ...
      val left4 = javaList(1, 2, 3)
      val e4 = intercept[exceptions.TestFailedException] {
        (left4 should contain allElementsOf Seq(1, 2, 3)) (equality)
      }
      checkShouldContainStackDepth(e4, left4, Seq(1, 2, 3), thisLineNumber - 2)
      ...
      val left5 = Map(1 -> "one", 2 -> "two", 3 -> "three")
      val e5 = intercept[exceptions.TestFailedException] {
        (left5 should contain allElementsOf Seq(1 -> "one", 2 -> "two", 3 -> "three")) (mapEquality)
      }
      checkShouldContainStackDepth(e5, left5, Seq(1 -> "one", 2 -> "two", 3 -> "three"), thisLineNumber - 2)
    ...
    def `should throw TestFailedException with correct stack depth and message when 'should not contain custom matcher' failed with custom explicit equality` {
      ...
      val left1 = List("1 ", "2", "3 ")
      val e1 = intercept[exceptions.TestFailedException] {
        (left1 should not contain allElementsOf (Seq("1", "2 ", "3"))) (trimEquality)
      }
      checkShouldNotContainStackDepth(e1, left1, Seq("1", "2 ", "3"), thisLineNumber - 2)

      val left2 = Set("1 ", "2", "3 ")
      val e2 = intercept[exceptions.TestFailedException] {
        (left2 should not contain allElementsOf (Seq("1", "2 ", "3"))) (trimEquality)
      }
      checkShouldNotContainStackDepth(e2, left2, Seq("1", "2 ", "3"), thisLineNumber - 2)

      val left3 = Array("1 ", "2", "3 ")
      val e3 = intercept[exceptions.TestFailedException] {
        (left3 should not contain allElementsOf (Seq("1", "2 ", "3"))) (trimEquality)
      }
      checkShouldNotContainStackDepth(e3, left3, Seq("1", "2 ", "3"), thisLineNumber - 2)

      val left4 = javaList("1 ", "2", "3 ")
      val e4 = intercept[exceptions.TestFailedException] {
        (left4 should not contain allElementsOf (Seq("1", "2 ", "3"))) (trimEquality)
      }
      checkShouldNotContainStackDepth(e4, left4, Seq("1", "2 ", "3"), thisLineNumber - 2)
      ...
      val left5 = Map(1 -> "one ", 2 -> "two", 3 -> "three ")
      val e5 = intercept[exceptions.TestFailedException] {
        (left5 should not contain allElementsOf (Seq(1 -> "one", 2 -> "two ", 3 -> "three"))) (mapTrimEquality)
      }
      checkShouldNotContainStackDepth(e5, left5, Seq(1 -> "one", 2 -> "two ", 3 -> "three"), thisLineNumber - 2)
