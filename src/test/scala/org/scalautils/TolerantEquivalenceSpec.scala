...
class TolerantEquivalenceSpec extends Spec with TypeRestrictedTripleEquals with TolerantEquivalence {
  ...
  object `The X +- Y syntax` {
    ...
    def `should throw IllegalArgumentException if the number passed to the right is 0 or negative for Float` {
      ...
      assert(caught7.getMessage === "-0.2 passed to tolerantEquivalence was zero or negative. Must be a positive non-zero number.")
    }
