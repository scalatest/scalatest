 ...
 * either a 'name' or 'glob' attribute:
 * </p>
 *
 * <pre>
 *   &lt;scalatest&gt;
...
class ScalaTestAntTask extends Task {
  ...
  //
  // Adds '-t testname' or '-z testglob' argument to args list for
  ...
  private def addTestArgs(args: ListBuffer[String]) {
    for (test <- tests) {
      ...
      else if (test.getGlob != null) {
        args += "-z"
        args += test.getGlob
      }
      else
        throw new BuildException(
          "missing name or glob attribute for <test> element")
    }
  }
