...
private[scalatest] object SuiteDiscoveryHelper {
  ...
  //
  // Tests are specified either by name or glob.  This method
                    ...
                    loader:           ClassLoader): List[SuiteParam] =
  {
    ...
    if (!testSpecs.isEmpty) {
      val names: Set[String] =
        testSpecs.filter(_.isGlob == false).map(_.spec).toSet
  
      val globs: Set[String] =
        testSpecs.filter(_.isGlob == true).map(_.spec).toSet
      ...
      for (suiteName <- accessibleSuites) {
        ...
        val globMatches: Set[String] =
          globs.filter(glob =>
            suiteInstance.testNames.exists(_.contains(glob)))
  
        if ((nameMatches.size > 0) || (globMatches.size > 0))
          buf += SuiteParam(suiteName,
                            nameMatches.toList.sortWith(_<_).toArray,
                            globMatches.toList.sortWith(_<_).toArray,
                            Array.empty)
      }
    }
