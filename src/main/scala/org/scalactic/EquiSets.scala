...
class EquiSets[T](val hashingEquality: HashingEquality[T]) { thisEquiSets =>
  case class HashBox(value: T) {
    override def equals(o: Any): Boolean = 
      o match {
        case other: HashBox => hashingEquality.areEqual(value, other.value)
        case _ => false
      }
    override def hashCode: Int = hashingEquality.hashCodeFor(value)
    override def toString: String = s"HashBox(${value.toString})"
  }
  class EquiSet private (private val underlying: Set[HashBox]) {
    def + (elem: T): thisEquiSets.EquiSet = new EquiSet(underlying + HashBox(elem))
    def + (elem1: T, elem2: T, elem3: T*): thisEquiSets.EquiSet =
      new EquiSet(underlying + (HashBox(elem1), HashBox(elem2), elem3.map(HashBox(_)): _*))
    def - (elem: T): thisEquiSets.EquiSet = new EquiSet(underlying - HashBox(elem))
    def - (elem1: T, elem2: T, elem3: T*): thisEquiSets.EquiSet =
      new EquiSet(underlying - (HashBox(elem1), HashBox(elem2), elem3.map(HashBox(_)): _*))
    ...
    def toSet: Set[thisEquiSets.HashBox] = underlying
  ...
  object EquiSet {
    ...
    def apply(elems: T*): EquiSet = 
      new EquiSet(Set(elems.map(HashBox(_)): _*))
  }
