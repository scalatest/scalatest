...
trait EnabledEquality extends LowPriorityEnabledEqualityConstraints {
  ...
  implicit override def enabledIdentityEqualityForType[A](implicit equivalenceOfA: Equivalence[A], ev: EnabledIdentityEquality[A]): EqualityConstraint[A, A] = new IdentityEqualityConstraint[A](equivalenceOfA)
}
