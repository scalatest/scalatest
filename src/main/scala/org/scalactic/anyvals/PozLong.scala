 ...
 */
  ...
  */
  ...
  // No point to call abs on a PozLong.
  def max(that: PozLong): PozLong = if (math.max(value, that.value) == value) this else that
  def min(that: PozLong): PozLong = if (math.min(value, that.value) == value) this else that
}
