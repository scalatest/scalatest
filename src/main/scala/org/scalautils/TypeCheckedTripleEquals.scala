...
trait TypeCheckedTripleEquals extends LowPriorityTypeCheckedConstraint {
  ...
  implicit def convertEquivalenceToBToATypeConstraint[A, B](equivalenceOfA: Equivalence[A])(implicit ev: B <:< A): TripleEqualsConstraint[A, B] = new BToAEquivalenceConstraint[A, B](equivalenceOfA, ev)
