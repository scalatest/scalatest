...
/*
val x: (Int Side String) Or ErrorMessage
...
class SideSpec extends UnitSpec with Accumulation with TypeCheckedTripleEquals {
  ...
  it can "have its non-inferred type widened by an apply call with a type param" in {
    ...
    */
    ...
    (West(3): AnyVal Side String) shouldBe West(3)
    (East("oops"): Int Side AnyRef) shouldBe East("oops")
  }
  it can "be used in infix notation" in {
    def div(a: Int, b: Int): Int Side ArithmeticException = {
      try West(a / b)
  ...
  // SKIP-SCALATESTJS-END
  "The Side companion" should "offer a concise type lambda syntax" in {
    import scala.language.higherKinds
    trait Functor[Context[_]] {
      def map[A, B](ca: Context[A])(f: A => B): Context[B]
    }
    class WestSideFunctor[WHITE] extends Functor[Side.E[WHITE]#UU] {
      override def map[B, C](ca: B Side WHITE)(f: B => C): C Side WHITE = ???
    }
    class EastSideFunctor[BLACK] extends Functor[Side.UU[BLACK]#E] {
