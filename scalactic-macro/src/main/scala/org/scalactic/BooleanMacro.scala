...
private[org] class BooleanMacro[C <: Context](val context: C, helperName: String) {
   ...
   */
  def binaryMacroBool(select: Select, prettifier: Tree, differ: Tree): Apply =
    Apply(
      Select(
        Select(
          Select(
   ...
   */
  def binaryMacroBool(select: Select, secondArg: Tree, prettifier: Tree, differ: Tree): Apply =
    Apply(
      Select(
        Select(
          Select(
   ...
   */
  def typedBinaryMacroBool(select: Select, typeArgs: List[Tree], prettifier: Tree, differ: Tree): Apply =
    Apply(
      Select(
        Select(
          Select(
  ...
  def traverseSelect(select: Select, rightExpr: Tree, prettifierTree: Tree, differTree: Tree): (Tree, Tree) = {
    ...
    if (logicOperators.contains(operator)) {
      val leftTree = // traverse the qualifier
        select.qualifier match {
          case selectApply: Apply => transformAst(selectApply.duplicate, prettifierTree, differTree)
          case selectSelect: Select => transformAst(selectSelect.duplicate, prettifierTree, differTree)
          case _ => simpleMacroBool(select.qualifier.duplicate, getText(select.qualifier), prettifierTree)
        }
      val rightTree = {
        val evalBlock =
          rightExpr match {
            case argApply: Apply => transformAst(argApply.duplicate, prettifierTree, differTree)
            case argSelect: Select => transformAst(argSelect, prettifierTree, differTree)
            case argTypeApply: TypeApply => transformAst(argTypeApply.duplicate, prettifierTree, differTree)
   ...
   */
  def transformAst(tree: Tree, prettifierTree: Tree, differTree: Tree): Tree = {
    tree match {
      case apply: Apply if apply.args.size == 1 =>
        apply.fun match {
          case select: Select if isSupportedBinaryOperator(select.name.decoded) =>
            val operator = select.name.decoded
            val (leftTree, rightTree) =  traverseSelect(select, apply.args(0), prettifierTree, differTree)
            operator match {
              case "==" =>
                leftTree match {
                    ...
                    leftApply.fun match {
                      ...
                      case _ =>
                        ...
                        Block(
                          ...
                          valDef("$org_scalatest_assert_macro_right", rightTree),
                          binaryMacroBool(select.duplicate, prettifierTree, differTree)
                        )
                    }
          ...
          case funApply: Apply if funApply.args.size == 1 =>
            funApply.fun match {
              case select: Select if select.name.decoded == "===" || select.name.decoded == "!==" =>
                val (leftTree, rightTree) = traverseSelect(select, funApply.args(0), prettifierTree, differTree)
                /**
                ...
                Block(
                  valDef("$org_scalatest_assert_macro_left", leftTree),
                  valDef("$org_scalatest_assert_macro_right", rightTree),
                  binaryMacroBool(select.duplicate, apply.args(0).duplicate, prettifierTree, differTree) // TODO: should apply.args(0) be traversed also?
                )
            ...
            funTypeApply.fun match {
              case select: Select if isSupportedBinaryOperator(select.name.decoded) =>
                ...
                Block(
                  valDef("$org_scalatest_assert_macro_left", leftTree),
                  valDef("$org_scalatest_assert_macro_right", rightTree),
                  typedBinaryMacroBool(select.duplicate, funTypeApply.args, prettifierTree, differTree)
                )
          ...
          if (select.name.decoded == "unary_!") {
            ...
            val leftTree =
              select.qualifier match {
                case selectApply: Apply => transformAst(selectApply.duplicate, prettifierTree, differTree)
                case selectSelect: Select => transformAst(selectSelect.duplicate, prettifierTree, differTree)
                case selectTypeApply: TypeApply => transformAst(selectTypeApply.duplicate, prettifierTree, differTree)
                case _ => simpleMacroBool(select.qualifier.duplicate, getText(select.qualifier), prettifierTree)
              }
   ...
   */
  def callHelper(methodName: String, clueTree: Tree, prettifierTree: Tree, posTree: Tree, differTree: Tree): Apply =
    Apply(
      Select(
        Ident(newTermName(helperName)),
        newTermName(methodName)
      ),
      List(Ident(newTermName("$org_scalatest_assert_macro_expr")), clueTree, prettifierTree, posTree, differTree)
    )
   ...
   * }
   */
  def genMacro[T](booleanExpr: Expr[Boolean], methodName: String, clueExpr: Expr[Any], prettifierExpr: Expr[_], posExpr: Expr[_], differExpr: Expr[_]): Expr[T] = {
    val ownerRepair = new MacroOwnerRepair[context.type](context)
    val expandedCode =
      context.Expr(
        Block(
          valDef("$org_scalatest_assert_macro_expr", transformAst(booleanExpr.tree, prettifierExpr.tree, differExpr.tree)),
          callHelper(methodName, clueExpr.tree, prettifierExpr.tree, posExpr.tree, differExpr.tree)
        )
      )
  ...
  /**
    ...
    * }
    */
  def genMacro[T](booleanExpr: Expr[Boolean], methodName: String, clueExpr: Expr[Any], prettifierExpr: Expr[_], differExpr: Expr[_]): Expr[T] = {
    val ownerRepair = new MacroOwnerRepair[context.type](context)
    val expandedCode =
      context.Expr(
        Block(
