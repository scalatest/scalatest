...
class StackDepthExceptionSpec extends FunSpec with Matchers with TableDrivenPropertyChecks {
  ...
  def positionExamples = 
    Table(
      "exception",
      new TestFailedException((_: StackDepthException) => Some("message"), None, Left(source.Position.here), None, None),
      // SKIP-SCALATESTJS-START
  ...
  def stackDepthExamples = 
    Table(
      "exception",
      new TestFailedException((_: StackDepthException) => Some("message"), None, Right((_: StackDepthException) => 3), None, None),
      // SKIP-SCALATESTJS-START
