...
class ContainWordSpec extends Spec {
  ...
  object `ContainWord ` {
    ...
    object `oneElementOf(col) method returns MatcherFactory1` {
      ...
      def `should have correct MatcherResult` {
        mr should have (
          ...
          'rawFailureMessage (Resources.rawDidNotContainOneElementOf),
          'rawNegatedFailureMessage (Resources.rawContainedOneElementOf),
          'rawMidSentenceFailureMessage (Resources.rawDidNotContainOneElementOf),
          'rawMidSentenceNegatedFailureMessage (Resources.rawContainedOneElementOf),
      ...
      def `should have correct negated MatcherResult` {
        nmr should have (
          ...
          'rawFailureMessage (Resources.rawContainedOneElementOf),
          'rawNegatedFailureMessage (Resources.rawDidNotContainOneElementOf),
          'rawMidSentenceFailureMessage (Resources.rawContainedOneElementOf),
          'rawMidSentenceNegatedFailureMessage (Resources.rawDidNotContainOneElementOf),
          'failureMessageArgs(Vector(lhs, Seq(2, 8))),
          'negatedFailureMessageArgs(Vector(lhs, Seq(2, 8))),
          'midSentenceFailureMessageArgs(Vector(lhs, Seq(2, 8))),
          'midSentenceNegatedFailureMessageArgs(Vector(lhs, Seq(2, 8)))    
    ...
    object `atLeastOneElementOf(Any*) method returns MatcherFactory1` {

      val mtf = contain atLeastOneElementOf List(1, 2)
      val mt = mtf.matcher[List[Int]]

      def `should have pretty toString` {
        mtf.toString should be ("contain atLeastOneElementOf List(1, 2)")
        mt.toString should be ("contain atLeastOneElementOf List(1, 2)")
      }
      ...
      def `should have correct MatcherResult` {
        mr should have (
          'matches (true),
          'failureMessage (lhs + " did not contain at least one element of List(1, 2)"),
          'negatedFailureMessage (lhs + " contained at least one element of List(1, 2)"),
          'midSentenceFailureMessage (lhs + " did not contain at least one element of List(1, 2)"),
          'midSentenceNegatedFailureMessage (lhs + " contained at least one element of List(1, 2)"),
          'rawFailureMessage ("{0} did not contain at least one element of {1}"),
          'rawNegatedFailureMessage ("{0} contained at least one element of {1}"),
          'rawMidSentenceFailureMessage ("{0} did not contain at least one element of {1}"),
          'rawMidSentenceNegatedFailureMessage ("{0} contained at least one element of {1}"),
      ...
      def `should have correct negated MatcherResult` {
        nmr should have (
          'matches (false),
          'failureMessage (lhs + " contained at least one element of List(1, 2)"),
          'negatedFailureMessage (lhs + " did not contain at least one element of List(1, 2)"),
          'midSentenceFailureMessage (lhs + " contained at least one element of List(1, 2)"),
          'midSentenceNegatedFailureMessage (lhs + " did not contain at least one element of List(1, 2)"),
          'rawFailureMessage ("{0} contained at least one element of {1}"),
          'rawNegatedFailureMessage ("{0} did not contain at least one element of {1}"),
          'rawMidSentenceFailureMessage ("{0} contained at least one element of {1}"),
          'rawMidSentenceNegatedFailureMessage ("{0} did not contain at least one element of {1}"),
          'failureMessageArgs(Vector(lhs, List(1, 2))),
          'negatedFailureMessageArgs(Vector(lhs, List(1, 2))),
          'midSentenceFailureMessageArgs(Vector(lhs, List(1, 2))),
          'midSentenceNegatedFailureMessageArgs(Vector(lhs, List(1, 2)))
    ...
    object `inOrderElementsOf(GenTraversable) method returns MatcherFactory1` {

      val mtf = contain inOrderElementsOf List(1, 2)
      val mt = mtf.matcher[List[Int]]

      def `should have pretty toString` {
        mtf.toString should be ("contain inOrderElementsOf (List(1, 2))")
        mt.toString should be ("contain inOrderElementsOf (List(1, 2))")
      }
    ...
    object `atMostOneElementOf(elements) method returns MatcherFactory1` {

      val mtf = contain atMostOneElementOf List(1, 2)
      val mt = mtf.matcher[List[Int]]

      def `should have pretty toString` {
        mtf.toString should be ("contain atMostOneElementOf (List(1, 2))")
        mt.toString should be ("contain atMostOneElementOf (List(1, 2))")
      }
      ...
      def `should have correct MatcherResult` {
        mr should have (
          'matches (true),
          'failureMessage (lhs + " did not contain at most one element of List(1, 2)"),
          'negatedFailureMessage (lhs + " contained at most one element of List(1, 2)"),
          'midSentenceFailureMessage (lhs + " did not contain at most one element of List(1, 2)"),
          'midSentenceNegatedFailureMessage (lhs + " contained at most one element of List(1, 2)"),
          'rawFailureMessage ("{0} did not contain at most one element of {1}"),
          'rawNegatedFailureMessage ("{0} contained at most one element of {1}"),
          'rawMidSentenceFailureMessage ("{0} did not contain at most one element of {1}"),
          'rawMidSentenceNegatedFailureMessage ("{0} contained at most one element of {1}"),
          'failureMessageArgs(Vector(lhs, List(1, 2))),
          'negatedFailureMessageArgs(Vector(lhs, List(1, 2))),
          'midSentenceFailureMessageArgs(Vector(lhs, List(1, 2))),
          'midSentenceNegatedFailureMessageArgs(Vector(lhs, List(1, 2)))
      ...
      def `should have correct negated MatcherResult` {
        nmr should have (
          'matches (false),
          'failureMessage (lhs + " contained at most one element of List(1, 2)"),
          'negatedFailureMessage (lhs + " did not contain at most one element of List(1, 2)"),
          'midSentenceFailureMessage (lhs + " contained at most one element of List(1, 2)"),
          'midSentenceNegatedFailureMessage (lhs + " did not contain at most one element of List(1, 2)"),
          'rawFailureMessage ("{0} contained at most one element of {1}"),
          'rawNegatedFailureMessage ("{0} did not contain at most one element of {1}"),
          'rawMidSentenceFailureMessage ("{0} contained at most one element of {1}"),
          'rawMidSentenceNegatedFailureMessage ("{0} did not contain at most one element of {1}"),
          'failureMessageArgs(Vector(lhs, List(1, 2))),
          'negatedFailureMessageArgs(Vector(lhs, List(1, 2))),
          'midSentenceFailureMessageArgs(Vector(lhs, List(1, 2))),
          'midSentenceNegatedFailureMessageArgs(Vector(lhs, List(1, 2)))
