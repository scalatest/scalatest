...
class EveryShouldContainInOrderElementsOfLogicalOrSpec extends Spec {
  ...
  object `every of Everys` {
    ...
    object `when used with (be xx and contain inOrderElementsOf xx)` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (be_== (Many(0, 1, 2, 2, 3)) or contain inOrderElementsOf Seq(1, 2, 3))
        all (list1s) should (be_== (Many(8, 2, 3, 4)) or contain inOrderElementsOf Seq(1, 2, 3))
        all (list1s) should (be_== (Many(0, 1, 2, 2, 3)) or contain inOrderElementsOf Seq(2, 3, 4))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (be_== (Many(8, 2, 3, 4)) or contain inOrderElementsOf Seq(2, 3, 4))
        }
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (be_== (Many("he", "hi", "hello")) or contain inOrderElementsOf Seq("HI", "HELLO"))
        all (hiLists) should (be_== (Many("ho", "hello")) or contain inOrderElementsOf Seq("HI", "HELLO"))
        all (hiLists) should (be_== (Many("he", "hi", "hello")) or contain inOrderElementsOf Seq("HELLO", "HI"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be_== (Many("ho", "hello")) or contain inOrderElementsOf Seq("HELLO", "HI"))
        }
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (be_== (Many("he", "hi", "hello")) or contain inOrderElementsOf Seq("HI", "HELLO"))) (decided by upperCaseStringEquality)
        (all (hiLists) should (be_== (Many("ho", "hello")) or contain inOrderElementsOf Seq("HI", "HELLO"))) (decided by upperCaseStringEquality)
        (all (hiLists) should (be_== (Many("he", "hi", "hello")) or contain inOrderElementsOf Seq("HELLO", "HI"))) (decided by upperCaseStringEquality)

        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (be_== (Many("ho", "hello")) or contain inOrderElementsOf Seq("HELLO", "HI"))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was not equal to " + decorateToStringValue(Many("ho", "hello")) + ", and " + decorateToStringValue(hiLists(0)) + " did not contain all elements of " + decorateToStringValue(Seq("HELLO", "HI")) + " in order", thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (be_== (Many(0, 1, 2, 2, 3)) or contain inOrderElementsOf Seq(1, 2, 2, 3))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("inOrderElementsOfDuplicate")))
    ...
    object `when used with (not be xx and not contain inOrderElementsOf xx)` {
      ...
      def `should do nothing if valid, else throw a TFE with an appropriate error message` {
        all (list1s) should (not be_== (One(2)) or not contain inOrderElementsOf (Seq(8, 3, 4)))
        all (list1s) should (not be_== (Many(0, 1, 2, 2, 3)) or not contain inOrderElementsOf (Seq(8, 3, 4)))
        all (list1s) should (not be_== (One(2)) or not contain inOrderElementsOf (Seq(1, 2, 3)))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (not be_== (Many(0, 1, 2, 2, 3)) or not contain inOrderElementsOf (Seq(1, 2, 3)))
        }
      ...
      def `should use the implicit Equality in scope` {
        ...
        all (hiLists) should (not be_== (Many("hello", "ho")) or not contain inOrderElementsOf (Seq("HELLO", "HO")))
        all (hiLists) should (not be_== (Many("he", "hi", "hello")) or not contain inOrderElementsOf (Seq("HELLO", "HO")))
        all (hiLists) should (not be_== (Many("hello", "ho")) or not contain inOrderElementsOf (Seq("HI", "HELLO")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (Many("he", "hi", "hello")) or not contain inOrderElementsOf (Seq("HI", "HELLO")))
        }
      ...
      def `should use an explicitly provided Equality` {
        (all (hiLists) should (not be_== (Many("hello", "ho")) or not contain inOrderElementsOf (Seq("HELLO", "HO")))) (decided by upperCaseStringEquality)
        (all (hiLists) should (not be_== (Many("he", "hi", "hello")) or not contain inOrderElementsOf (Seq("HELLO", "HO")))) (decided by upperCaseStringEquality)
        (all (hiLists) should (not be_== (Many("hello", "ho")) or not contain inOrderElementsOf (Seq("HI", "HELLO")))) (decided by upperCaseStringEquality)

        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (not be_== (Many("he", "hi", "hello")) or not contain inOrderElementsOf (Seq("HI", "HELLO")))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was equal to " + decorateToStringValue(Many("he", "hi", "hello")) + ", and " + decorateToStringValue(hiLists(0)) + " contained all elements of " + decorateToStringValue(Seq("HI", "HELLO")) + " in order", thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
      ...
      def `should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value` {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (not be_== (One(2)) or not contain inOrderElementsOf (Seq(1, 2, 2, 3)))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("inOrderElementsOfDuplicate")))
