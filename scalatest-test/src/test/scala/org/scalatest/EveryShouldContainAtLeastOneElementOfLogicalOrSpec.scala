...
class EveryShouldContainAtLeastOneElementOfLogicalOrSpec extends FreeSpec {
  ...
  "every of Everys" - {
    ...
    "when used with (be (...) and contain atLeastOneElementOf (...)) syntax" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (be (One(1)) or contain atLeastOneElementOf Seq(1, 3, 4))
        all (list1s) should (be (One(2)) or contain atLeastOneElementOf Seq(1, 3, 4))
        all (list1s) should (be (One(1)) or contain atLeastOneElementOf Seq(2, 3, 4))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (be (One(2)) or contain atLeastOneElementOf Seq(2, 3, 8))
        }
      ...
      "should use the implicit Equality in scope" in {
        ...
        all (hiLists) should (be (One("hi")) or contain atLeastOneElementOf Seq("HI", "HE"))
        all (hiLists) should (be (One("ho")) or contain atLeastOneElementOf Seq("HI", "HE"))
        all (hiLists) should (be (One("hi")) or contain atLeastOneElementOf Seq("hi", "he"))
      ...
      "should allow RHS to contain duplicated value" in {
        all (list1s) should (be (One(1)) or contain atLeastOneElementOf Seq(3, 2, 2, 1))
      }
    }
    ...
    "when used with (not be (...) and not contain atLeastOneElementOf (...))" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (not be (One(2)) or not contain atLeastOneElementOf (Seq(8, 3, 4)))
        all (list1s) should (not be (One(1)) or not contain atLeastOneElementOf (Seq(8, 3, 4)))
        all (list1s) should (not be (One(2)) or not contain atLeastOneElementOf (Seq(8, 3, 1)))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (not be (One(1)) or not contain atLeastOneElementOf (Seq(2, 3, 1)))
        }
      ...
      "should use the implicit Equality in scope" in {
        ...
        all (hiLists) should (not be (One("ho")) or not contain atLeastOneElementOf (Seq("hi", "he")))
        all (hiLists) should (not be (One("hi")) or not contain atLeastOneElementOf (Seq("hi", "he")))
        all (hiLists) should (not be (One("ho")) or not contain atLeastOneElementOf (Seq("HI", "HE")))
      ...
      "should allow RHS to contain duplicated value" in {
        all (list1s) should (not be (One(2)) or not contain atLeastOneElementOf (Seq(3, 2, 2, 1)))
      }
    }
  }
}
