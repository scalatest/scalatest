...
class ListShouldContainAtMostOneElementOfLogicalOrSpec extends FreeSpec {
  ...
  "collection of Lists" - {
    ...
    "when used with (be (...) and contain oneOf (...)) syntax" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (be_== (List(1, 2)) or contain atMostOneElementOf Seq(1, 6, 8))
        all (list1s) should (be_== (List(2, 3)) or contain atMostOneElementOf Seq(1, 6, 8))
        all (list1s) should (be_== (List(1, 2)) or contain atMostOneElementOf Seq(1, 2, 3))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (be_== (List(2, 3)) or contain atMostOneElementOf Seq(1, 2, 3))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(list1s(0)) + " was not equal to " + decorateToStringValue(List(2, 3)) + ", and " + decorateToStringValue(list1s(0)) + " did not contain at most one element of " + decorateToStringValue(Seq(1, 2, 3)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      "should use the implicit Equality in scope" in {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (be_== (List("hi", "he")) or contain atMostOneElementOf Seq("HI", "HO"))
        all (hiLists) should (be_== (List("HO")) or contain atMostOneElementOf Seq("HI", "HO"))
        all (hiLists) should (be_== (List("hi", "he")) or contain atMostOneElementOf Seq("HI", "HE"))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (be_== (List("HO")) or contain atMostOneElementOf Seq("HI", "HE"))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was not equal to " + decorateToStringValue(List("HO")) + ", and " + decorateToStringValue(hiLists(0)) + " did not contain at most one element of " + decorateToStringValue(Seq("HI", "HE")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }

      "should use an explicitly provided Equality" in {
        (all (hiLists) should (be_== (List("hi", "he")) or contain atMostOneElementOf Seq("HI", "HO"))) (decided by upperCaseStringEquality)
        (all (hiLists) should (be_== (List("HO")) or contain atMostOneElementOf Seq("HI", "HO"))) (decided by upperCaseStringEquality)
        (all (hiLists) should (be_== (List("hi", "he")) or contain atMostOneElementOf Seq("HI", "HE"))) (decided by upperCaseStringEquality)

        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (be_== (List("HO")) or contain atMostOneElementOf Seq("HI", "HE"))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was not equal to " + decorateToStringValue(List("HO")) + ", and " + decorateToStringValue(hiLists(0)) + " did not contain at most one element of " + decorateToStringValue(Seq("HI", "HE")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
      ...
      "should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value" in {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (be_== (List(1, 2)) or contain atMostOneElementOf Seq(1, 2, 2, 3))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("atMostOneElementOfDuplicate")))
    ...
    "when used with (not be (...) and not contain atMostOneElementOf Seq(...))" - {
      ...
      "should do nothing if valid, else throw a TFE with an appropriate error message" in {
        all (list1s) should (not be_== (List(2, 3)) or not contain atMostOneElementOf (Seq(1, 2, 3)))
        all (list1s) should (not be_== (List(1, 2)) or not contain atMostOneElementOf (Seq(1, 2, 3)))
        all (list1s) should (not be_== (List(2, 3)) or not contain atMostOneElementOf (Seq(2, 3, 4)))

        val e1 = intercept[TestFailedException] {
          all (list1s) should (not be_== (List(1, 2)) or not contain atMostOneElementOf (Seq(2, 3, 4)))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(list1s(0)) + " was equal to " + decorateToStringValue(List(1, 2)) + ", and " + decorateToStringValue(list1s(0)) + " contained at most one element of " + decorateToStringValue(Seq(2, 3, 4)), thisLineNumber - 2, list1s), fileName, thisLineNumber - 2)
      }

      "should use the implicit Equality in scope" in {
        implicit val ise = upperCaseStringEquality

        all (hiLists) should (not be_== (List("ho")) or not contain atMostOneElementOf (Seq("HI", "HE")))
        all (hiLists) should (not be_== (List("hi", "he")) or not contain atMostOneElementOf (Seq("HE", "HI")))
        all (hiLists) should (not be_== (List("ho")) or not contain atMostOneElementOf (Seq("HI", "HE")))

        val e1 = intercept[TestFailedException] {
          all (hiLists) should (not be_== (List("hi", "he")) or not contain atMostOneElementOf (Seq("HI", "HO")))
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was equal to " + decorateToStringValue(List("hi", "he")) + ", and " + decorateToStringValue(hiLists(0)) + " contained at most one element of " + decorateToStringValue(Seq("HI", "HO")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }

      "should use an explicitly provided Equality" in {
        (all (hiLists) should (not be_== (List("ho")) or not contain atMostOneElementOf (Seq("HI", "HE")))) (decided by upperCaseStringEquality)
        (all (hiLists) should (not be_== (List("hi", "he")) or not contain atMostOneElementOf (Seq("HE", "HI")))) (decided by upperCaseStringEquality)
        (all (hiLists) should (not be_== (List("ho")) or not contain atMostOneElementOf (Seq("HI", "HE")))) (decided by upperCaseStringEquality)

        val e1 = intercept[TestFailedException] {
          (all (hiLists) should (not be_== (List("hi", "he")) or not contain atMostOneElementOf (Seq("HI", "HO")))) (decided by upperCaseStringEquality)
        }
        checkMessageStackDepth(e1, allErrMsg(0, decorateToStringValue(hiLists(0)) + " was equal to " + decorateToStringValue(List("hi", "he")) + ", and " + decorateToStringValue(hiLists(0)) + " contained at most one element of " + decorateToStringValue(Seq("HI", "HO")), thisLineNumber - 2, hiLists), fileName, thisLineNumber - 2)
      }
      ...
      "should throw NotAllowedException with correct stack depth and message when RHS contain duplicated value" in {
        val e1 = intercept[exceptions.NotAllowedException] {
          all (list1s) should (not be_== (List(2, 3)) or not contain atMostOneElementOf (Seq(1, 2, 2, 3)))
        }
        e1.failedCodeFileName.get should be (fileName)
        e1.failedCodeLineNumber.get should be (thisLineNumber - 3)
        e1.message should be (Some(FailureMessages("atMostOneElementOfDuplicate")))
