...
/**
 * A sister class to <code>org.scalatest.freespec.PathFreeSpec</code> that isolates tests by running each test in its own
 * instance of the test class, and for each test, only executing the <em>path</em> leading to that test.
 *
 * <p>
 * Class <code>PathFreeSpec</code> behaves similarly to class <code>org.scalatest.freespec.AnyFreeSpec</code>, except that tests
 * are isolated based on their path. The purpose of <code>PathFreeSpec</code> is to facilitate writing
 ...
 *
 * class ExampleSpec extends freespec.PathFreeSpec with Matchers {
 *
 *   "A ListBuffer" - {
 *
 ...
 * <code>org.scalatest.freespec.PathFreeSpec</code>. However, <code>PathFreeSpec</code> takes isolation one step further: a test
 * in a <code>PathFreeSpec</code> does not observe side effects performed outside tests in earlier blocks that do not
 ...
 *
 * <p>
 * <em>Note: class <code>PathFreeSpec</code>'s approach to isolation was inspired in part by the
 ...
 * there's one and only one way to do it in a <code>PathFreeSpec</code>: place the mutable objects lexically before
 ...
 *
 * <p>
 * The reason lexical placement is the one and only one way to share fixtures in a <code>PathFreeSpec</code> is because
 ...
 * a <code>PathFreeSpec</code>. You also can't override <code>withFixture</code>, because <code>PathFreeSpec</code>
 ...
 * a <code>PathFreeSpec</code>'s approach to isolation with other ways ScalaTest provides to share fixtures or
 * execute tests, because doing so could make the resulting test code hard to reason about. A
 * <code>PathFreeSpec</code>'s execution model is a bit magical, but because it executes in one and only one
 * way, users should be able to reason about the code.
 * To help you visualize how a <code>PathFreeSpec</code> is executed, consider the following variant of
 ...
 * import scala.collection.mutable.ListBuffer
 *
 * class ExampleSpec extends PathFreeSpec with Matchers {
 *
 ...
 *
 * <p>
 * Note that each test is executed in order of appearance in the <code>PathFreeSpec</code>, and that only
 ...
 *
 * <p>
 * To provide its special brand of test isolation, <code>PathFreeSpec</code> executes quite differently from its
 * sister class in <code>org.scalatest.freespec</code>. An <code>org.scalatest.freespec.PathFreeSpec</code>
 ...
 * running those tests. When <code>run</code> is invoked on a <code>PathFreeSpec</code>, it reports the registered
 * results and does not run the tests again. If <code>run</code> is invoked a second or third time, in fact,
 * a <code>PathFreeSpec</code> will each time report the same results registered during construction. If you want
 * to run the tests of a <code>PathFreeSpec</code> anew, you'll need to create a new instance and invoke
 * <code>run</code> on that.
 * <p>
 *
 * <p>
 * A <code>PathFreeSpec</code> will create one instance for each "leaf" node it contains. The main kind of leaf node is
 * a test, such as:
 * </p>
 *
 * <pre class="stHighlight">
 ...
 * <code>PathFreeSpec</code> instance must communicate with the other instances it creates for running any subsequent
 ...
 * of <code>PathFreeSpec</code> checks the thread-local variable. If the thread-local is not set, it knows it
 ...
 *
 * <p>
 * You mark a test as ignored in an <code>org.scalatest.freespec.PathFreeSpec</code> in the same manner as in
 ...
 *
 * <p>
 * You output information using <code>Informer</code>s in an <code>org.scalatest.freespec.PathFreeSpec</code> in the same manner
 ...
 *
 * <p>
 * You mark a test as pending in an <code>org.scalatest.freespec.PathFreeSpec</code> in the same manner as in
 ...
 *
 * <p>
 * You can place tests into groups by tagging them in an <code>org.scalatest.freespec.PathFreeSpec</code> in the same manner
 ...
 * time run is invoked, an <code>org.scalatest.freespec.PathFreeSpec</code> will always execute all non-ignored tests. When
 * <code>run</code> is invoked on a <code>PathFreeSpec</code>, if some tests are excluded based on tags, the registered
 * results of running those tests will not be reported. (But those tests will have already run and the results
 * registered.) By contrast, because an <code>org.scalatest.freespec.PathFreeSpec</code> only executes tests after <code>run</code>
 ...
 * In an <code>org.scalatest.freespec.PathFreeSpec</code>, by contrast, all non-ignored tests will be executed, each
 ...
 * you can sometimes exclude them during a run, you probably don't want to put them in a <code>PathFreeSpec</code>. Because
 ...
 * <p>
 * You can factor out shared tests in an <code>org.scalatest.freespec.PathFreeSpec</code> in the same manner as in
 ...
 * a <code>PathFreeSpec</code> executes tests eagerly at construction time, registering the results of those test runs
 * and reporting them later when <code>run</code> is invoked, the order of nested suites versus test runs would be
 * different in a <code>org.scalatest.freespec.PathFreeSpec</code> than in an <code>org.scalatest.freespec.AnyFreeSpec</code>. In
 * <code>org.scalatest.freespec.AnyFreeSpec</code>'s implementation of <code>run</code>, nested suites are executed then tests
 * are executed. A <code>org.scalatest.freespec.PathFreeSpec</code> with nested suites would execute these in the opposite
 * order: first tests then nested suites. To help make <code>PathFreeSpec</code> code easier to
 * reason about by giving readers of one less difference to think about, nested suites are not allowed. If you want
 * to add nested suites to a <code>PathFreeSpec</code>, you can instead wrap them all in a
 ...
 *
 * <p>
 * In the test completion events fired by a <code>PathFreeSpec</code> (<code>TestSucceeded</code>,
 ...
 * for running the tests of a <code>PathFreeSpec</code>, calculated by summing the durations of all the individual
...
class PathFreeSpec extends org.scalatest.freespec.PathFreeSpecLike {
// SKIP-SCALATESTJS,NATIVE-END
//SCALATESTJS,NATIVE-ONLY abstract class PathFreeSpec extends org.scalatest.freespec.PathFreeSpecLike {
