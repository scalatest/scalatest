...
trait Inspectors {
  ...
  def exists[E, C[_]](xs: C[E])(fun: E => Unit)(implicit collecting: Collecting[E, C[E]]) {
    doForAtLeast(1, collecting.genTraversableFrom(xs), xs, false, "Inspectors.scala", "exists", 0)(fun)
  }
...
private[scalatest] object InspectorsHelper {
        ...
        try {
          ...
          case e if !shouldPropagate(e) => 
            val messageKey = head match {
              ...
              case entry: org.scalactic.Entry[_, _] if xsIsMap => entry.getKey.toString
        ...
        sde => 
          Some(
            if (methodName == "exists") {
              if (passedCount > 0)
                Resources.existsFailed(min.toString, elementLabel(passedCount), indentErrorMessages(messageAcc).mkString(", \n"), decorateToStringValue(original))
              else
                Resources.existsFailedNoElement(min.toString, indentErrorMessages(messageAcc).mkString(", \n"), decorateToStringValue(original))
            }
            else {
              if (shorthand)
                if (passedCount > 0)
  ...
  def createMessageForThrowable(e: Throwable, element: Any, index: Int, original: Any): String = {
    ...
    createMessage(messageKey, e, xsIsMap)
  }

  def doForEvery[T](xs: GenTraversable[T], original: Any, shorthand: Boolean, sourceFileName: String, methodName: String, stackDepthAdjustment: Int, createMsg: (Throwable, Any, Int, Any) => String = createMessageForThrowable _)(fun: T => Unit) {
    @tailrec
