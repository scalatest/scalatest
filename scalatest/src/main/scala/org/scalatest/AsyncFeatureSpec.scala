...
In an AsyncFeatureSpec, threads that transform and perform callbacks for Futures
...
to use a synchronous style, such as FeatureSpec, which doesn't take threads fron
