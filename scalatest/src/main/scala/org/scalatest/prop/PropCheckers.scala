...
import org.scalatest.enablers.{CheckerAsserting, PropCheckerAsserting}
...
repeatedly pass generated data to the function. In this case, the test data is composed of integer lists named <code>a</code> and <code>b</code>.
  ...
  * @author Bill Venners
  */
trait PropCheckers extends Configuration {
  ...
  /**
    ...
    */
  def check[A1, ASSERTION](f: A1 => ASSERTION, configParams: PropertyCheckConfigParam*)
                             (implicit
    ...
    val propTest = PropertyTest.forAll1(List.empty, param)(f)
    asserting.check(propTest, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[A1, A2, ASSERTION](f: (A1, A2) => ASSERTION, configParams: PropertyCheckConfigParam*)
                                 (implicit
                                  config: PropertyCheckConfiguration,
                                  genA1: Generator[A1],
                                  genA2: Generator[A2],
    ...
    val param = getParameter(configParams, config)
    val propTest = PropertyTest.forAll2(List.empty, param)(f)
    asserting.check(propTest, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[A1, A2, A3, ASSERTION](f: (A1, A2, A3) => ASSERTION, configParams: PropertyCheckConfigParam*)
                                     (implicit
                                      config: PropertyCheckConfiguration,
                                      genA1: Generator[A1],
                                      genA2: Generator[A2],
    ...
    val param = getParameter(configParams, config)
    val propTest = PropertyTest.forAll3(List.empty, param)(f)
    asserting.check(propTest, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[A1, A2, A3, A4, ASSERTION](f: (A1, A2, A3, A4) => ASSERTION, configParams: PropertyCheckConfigParam*)
                                         (implicit
                                          config: PropertyCheckConfiguration,
                                          genA1: Generator[A1],
                                          genA2: Generator[A2],
    ...
    val param = getParameter(configParams, config)
    val propTest = PropertyTest.forAll4(List.empty, param)(f)
    asserting.check(propTest, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[A1, A2, A3, A4, A5, ASSERTION](f: (A1,A2,A3,A4,A5) => ASSERTION, configParams: PropertyCheckConfigParam*)
                                             (implicit
                                              config: PropertyCheckConfiguration,
                                              genA1: Generator[A1],
                                              genA2: Generator[A2],
    ...
    val param = getParameter(configParams, config)
    val propTest = PropertyTest.forAll5(List.empty, param)(f)
    asserting.check(propTest, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[A1, A2, A3, A4, A5, A6, ASSERTION](f: (A1, A2, A3, A4, A5, A6) => ASSERTION, configParams: PropertyCheckConfigParam*)
                                                 (implicit
                                                  config: PropertyCheckConfiguration,
                                                  genA1: Generator[A1],
                                                  genA2: Generator[A2],
    ...
    val param = getParameter(configParams, config)
    val propTest = PropertyTest.forAll6(List.empty, param)(f)
    asserting.check(propTest, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[ASSERTION](p: PropertyTest, param: Configuration.Parameter)(implicit asserting: PropCheckerAsserting[ASSERTION], prettifier: Prettifier, pos: source.Position, resultChecker: PropertyTestResultChecker[ASSERTION]): asserting.Result = {
    asserting.check(p, param, prettifier, pos, None)
  }
  ...
  /**
    ...
    * @throws TestFailedException if a test case is discovered for which the property doesn't hold.
    */
  def check[ASSERTION](p: PropertyTest, configParams: PropertyCheckConfigParam*)(implicit config: PropertyCheckConfiguration, asserting: PropCheckerAsserting[ASSERTION], prettifier: Prettifier, pos: source.Position, resultChecker: PropertyTestResultChecker[ASSERTION]): asserting.Result = {
    val param = getParameter(configParams, config)
    asserting.check(p, param, prettifier, pos, None)
  }
}
...
/**
  ...
  * @author Bill Venners
  */
object PropCheckers extends PropCheckers
