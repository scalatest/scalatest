...
case class Classification(val totalGenerated: PosInt, val totals: Map[String, Int]) {
  def proportions: Map[String, Double] =
    totals.mapValues(count => count.toDouble / totalGenerated.toDouble)
  def percentages: Map[String, Int] =
    totals.mapValues(count => count.toInt * 100 / totalGenerated.toInt)
