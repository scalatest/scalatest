...
In an AsyncWordSpec, threads that transform and perform callbacks for Futures
...
to use a synchronous style, such as WordSpec, which doesn't take threads fron
