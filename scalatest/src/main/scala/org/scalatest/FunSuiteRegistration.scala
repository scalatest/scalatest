...
trait FunSuiteRegistration extends Suite with TestRegistration with Informing with Notifying with Alerting with Documenting { thisSuite =>
  ...
  final def registerTest(testText: String, testTags: Tag*)(testFun: => Registration) {
    ...
    engine.registerTest(testText, transformToOutcome(testFun), Resources.testCannotBeNestedInsideAnotherTest, "FunSuiteRegistration.scala", "registerTest", 4, stackDepthAdjustment, None, None, None, testTags: _*)
  }

  final def registerIgnoredTest(testText: String, testTags: Tag*)(testFun: => Registration) {
    engine.registerIgnoredTest(testText, transformToOutcome(testFun), Resources.testCannotBeNestedInsideAnotherTest, "FunSuiteRegistration.scala", "registerIgnoredTest", 4, -3, None, testTags: _*)
  }
  ...
  protected def test(testName: String, testTags: Tag*)(testFun: => Registration) {
    ...
    //SCALATESTJS-ONLY val stackDepthAdjustment = -4
    engine.registerTest(testName, transformToOutcome(testFun), Resources.testCannotAppearInsideAnotherTest, "FunSuiteRegistration.scala", "test", stackDepth, stackDepthAdjustment, None, None, None, testTags: _*)
  }
  ...
  protected def ignore(testName: String, testTags: Tag*)(testFun: => Registration) {
    ...
    engine.registerIgnoredTest(testName, transformToOutcome(testFun), Resources.ignoreCannotAppearInsideATest, "FunSuiteRegistration.scala", "ignore", stackDepth, stackDepthAdjustment, None, testTags: _*)
  }
