...
trait FlatSpecLike extends Suite with TestRegistration with ShouldVerb with MustVerb with CanVerb with Informing with Notifying with Alerting with Documenting with Compatibility { thisSuite =>
  ...
  final def registerIgnoredTest(testText: String, testTags: Tag*)(testFun: FixtureParam => Assertion) {
    ...
    engine.registerIgnoredTest(testText, Transformer(testFun), Resources.testCannotBeNestedInsideAnotherTest, "FlatSpecLike.scala", "registerIgnoredTest", 4, stackDepthAdjustment, None, testTags: _*)
  }
   ...
   */
  private def registerTestToRun(specText: String, testTags: List[Tag], methodName: String, testFun: FixtureParam => Assertion) {
  ...
  protected final class ItVerbStringTaggedAs(verb: String, name: String, tags: List[Tag]) {
    ...
    def in(testFun: () => Assertion) {
      registerTestToRun(verb.trim + " " + name.trim, tags, "in", new NoArgTestWrapper[FixtureParam, Assertion](testFun))
    }
     ...
     * @param testFun the test function
     */
    def in(testFun: FixtureParam => Assertion) {
     ...
     * @param testFun the test function
     */
    def ignore(testFun: FixtureParam => Assertion) {
   ...
   * @throws NullArgumentException if <code>specText</code> or any passed test tag is <code>null</code>
   */
  private def registerTestToIgnore(specText: String, testTags: List[Tag], methodName: String, testFun: FixtureParam => Assertion) {
    // SKIP-SCALATESTJS-START
    val stackDepth = 4
