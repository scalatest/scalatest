...
trait Matchers extends Assertions with Tolerance with ShouldVerb with MatcherWords with Explicitly { matchers =>
  ...
  private def indicateSuccess(message: => String): Assertion = Succeeded

  private def indicateFailure(failureMessage: => String): Assertion =
    throw newTestFailedException(failureMessage)

  private def indicateFailure(shouldBeTrue: Boolean, failureMessage: => String, negatedFailureMessage: => String): Assertion =
    throw newTestFailedException(
      if (shouldBeTrue) failureMessage else negatedFailureMessage
    )

  private def indicateFailure(shouldBeTrue: Boolean, failureMessage: => String, negatedFailureMessage: => String, optionalCause: Option[Throwable] = None, stackDepthAdjustment: Int = 0): Assertion =
    throw newTestFailedException(
      if (shouldBeTrue) failureMessage else negatedFailureMessage,
      None,
  ...
  private def indicateFailure(failureMessage: => String, optionalCause: Option[Throwable], stackDepthAdjustment: Int): Assertion =
    throw newTestFailedException(
      failureMessage,
      optionalCause,
      stackDepthAdjustment
  ...
  private def indicateFailure(shouldBeTrue: Boolean, withFriendlyReminder: Boolean, failureMessageWithFriendlyReminder: => String, failureMessageWithoutFriendlyReminder: => String,
  ...
  class ResultOfBeWordForAny[T](left: T, shouldBeTrue: Boolean) {
    ...
    def theSameInstanceAs(right: AnyRef)(implicit toAnyRef: T <:< AnyRef): Assertion = {
      ...
      else indicateSuccess(FailureMessages.wasSameInstanceAs(left, right))
    }
    ...
    */
    ...
    def a(bePropertyMatcher: BePropertyMatcher[T])(implicit ev: T <:< AnyRef): Assertion = { // TODO: Try expanding this to 2.10 AnyVals
      ...
      } else indicateSuccess(FailureMessages.wasA(left, UnquotedString(result.propertyName)))
    }
    ...
    def an(beTrueMatcher: BePropertyMatcher[T])(implicit ev: T <:< AnyRef): Assertion = { // TODO: Try expanding this to 2.10 AnyVals
      ...
      } else indicateSuccess(FailureMessages.wasAn(left, UnquotedString(beTrueMatchResult.propertyName)))
    }
    ...
    def definedAt[U](right: U)(implicit ev: T <:< PartialFunction[U, _]): Assertion = {
      ...
      else indicateSuccess(FailureMessages.wasDefinedAt(left, right))
    }
  ...
  final class ResultOfIncludeWordForString(left: String, shouldBeTrue: Boolean) {
    ...
    def regex(rightRegex: Regex): Assertion = {
      ...
      else indicateSuccess(FailureMessages.includedRegex(left, rightRegex))
    }
  ...
  final class ResultOfStartWithWordForString(left: String, shouldBeTrue: Boolean) {
    ...
    def regex(rightRegex: Regex): Assertion = {
      ...
      else indicateSuccess(FailureMessages.startedWithRegex(left, rightRegex))
    }
  ...
  final class ResultOfEndWithWordForString(left: String, shouldBeTrue: Boolean) {
    ...
    def regex(rightRegex: Regex): Assertion = {
      ...
      else indicateSuccess(FailureMessages.endedWithRegex(left, rightRegex))
    }
  ...
  final class ResultOfFullyMatchWordForString(left: String, shouldBeTrue: Boolean) {
    ...
    def regex(rightRegex: Regex): Assertion = {
      ...
      else indicateSuccess(FailureMessages.fullyMatchedRegex(left, rightRegex))
    }
...
*/
  ...
  final class ResultOfHaveWordForExtent[A](left: A, shouldBeTrue: Boolean) {
    ...
    def length(expectedLength: Long)(implicit len: Length[A]): Assertion = {
      ...
      else indicateSuccess(FailureMessages.hadLength(left, expectedLength))
    }
    ...
    def size(expectedSize: Long)(implicit sz: Size[A]): Assertion = {
      ...
      else indicateSuccess(FailureMessages.hadSize(left, expectedSize))
    }
    ...
    def message(expectedMessage: String)(implicit messaging: Messaging[A]): Assertion = {
      ...
      else indicateSuccess(FailureMessages.hadExpectedMessage(left, expectedMessage))
    }
  ...
  sealed class ResultOfNotWordForCollectedAny[T](collected: Collected, xs: scala.collection.GenTraversable[T], original: Any, shouldBeTrue: Boolean) {
    ...
    def be(right: Any): Assertion = {
      doCollected(collected, xs, original, "be", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.wasEqualTo(e, right))
      }
    }
    ...
    def be[U >: T](resultOfAWordApplication: ResultOfAWordToBePropertyMatcherApplication[U]): Assertion = {
      doCollected(collected, xs, original, "be", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.wasA(e, UnquotedString(result.propertyName)))
      }
    }
    ...
    def be[U](resultOfDefinedAt: ResultOfDefinedAt[U])(implicit ev: T <:< PartialFunction[U, _]): Assertion = {
      doCollected(collected, xs, original, "be", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.wasDefinedAt(e, resultOfDefinedAt.right))
      }
    }
    ...
    def have(resultOfLengthWordApplication: ResultOfLengthWordApplication)(implicit len: Length[T]): Assertion = {
      doCollected(collected, xs, original, "have", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.hadLength(e, right))
      }
    }
    ...
    def have(resultOfSizeWordApplication: ResultOfSizeWordApplication)(implicit sz: Size[T]): Assertion = {
      doCollected(collected, xs, original, "have", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.hadSize(e, right))
      }
    }
    ...
    def have[U >: T](firstPropertyMatcher: HavePropertyMatcher[U, _], propertyMatchers: HavePropertyMatcher[U, _]*): Assertion = {
      doCollected(collected, xs, original, "have", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.allPropertiesHadExpectedValues(e))
      }
    }
    ...
    def contain(expectedElement: Any)(implicit containing: Containing[T]): Assertion = {
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedExpectedElement(e, right))
      }
    }
    ...
    def contain(oneOf: ResultOfOneOfApplication)(implicit containing: Containing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        if (containing.containsOneOf(e, right) != shouldBeTrue)
          indicateFailure(shouldBeTrue, FailureMessages.didNotContainOneOfElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))),
                          FailureMessages.containedOneOfElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))), None, innerStackDepth)
        else indicateSuccess(FailureMessages.containedOneOfElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(oneElementOf: ResultOfOneElementOfApplication)(implicit containing: Containing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedOneElementOf(e, right))
      }
    }
    ...
    def contain(atLeastOneOf: ResultOfAtLeastOneOfApplication)(implicit aggregating: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        if (aggregating.containsAtLeastOneOf(e, right) != shouldBeTrue)
          indicateFailure(shouldBeTrue, FailureMessages.didNotContainAtLeastOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))),
                          FailureMessages.containedAtLeastOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))), None, innerStackDepth)
        else indicateSuccess(FailureMessages.containedAtLeastOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(atLeastOneElementOf: ResultOfAtLeastOneElementOfApplication)(implicit evidence: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAtLeastOneElementOf(e, right))
      }
    }
    ...
    def contain(noneOf: ResultOfNoneOfApplication)(implicit containing: Containing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        if (containing.containsNoneOf(e, right) != shouldBeTrue)
          indicateFailure(shouldBeTrue, FailureMessages.containedAtLeastOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))),
                          FailureMessages.didNotContainAtLeastOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))), None, innerStackDepth)
        else indicateSuccess(FailureMessages.didNotContainAtLeastOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(noElementsOf: ResultOfNoElementsOfApplication)(implicit evidence: Containing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.didNotContainAtLeastOneElementOf(e, right))
      }
    }
    ...
    def contain(theSameElementsAs: ResultOfTheSameElementsAsApplication)(implicit aggregating: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedSameElements(e, right))
      }
    }
    ...
    def contain(theSameElementsInOrderAs: ResultOfTheSameElementsInOrderAsApplication)(implicit sequencing: Sequencing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedSameElementsInOrder(e, right))
      }
    }
    ...
    def contain(only: ResultOfOnlyApplication)(implicit aggregating: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedOnlyElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(only: ResultOfInOrderOnlyApplication)(implicit sequencing: Sequencing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        if (sequencing.containsInOrderOnly(e, right) != shouldBeTrue)
          indicateFailure(shouldBeTrue, FailureMessages.didNotContainInOrderOnlyElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))),
                          FailureMessages.containedInOrderOnlyElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))),
                          None, innerStackDepth)
        else indicateSuccess(FailureMessages.containedInOrderOnlyElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(only: ResultOfAllOfApplication)(implicit aggregating: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAllOfElements(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(only: ResultOfAllElementsOfApplication)(implicit evidence: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAllElementsOf(e, right))
      }
    }
    ...
    def contain(inOrder: ResultOfInOrderApplication)(implicit sequencing: Sequencing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAllOfElementsInOrder(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(inOrderElementsOf: ResultOfInOrderElementsOfApplication)(implicit evidence: Sequencing[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAllElementsOfInOrder(e, right))
      }
    }
    ...
    def contain(atMostOneOf: ResultOfAtMostOneOfApplication)(implicit aggregating: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAtMostOneOf(e, UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))))
      }
    }
    ...
    def contain(atMostOneElementOf: ResultOfAtMostOneElementOfApplication)(implicit evidence: Aggregating[T]): Assertion = {
      ...
      doCollected(collected, xs, original, "contain", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.containedAtMostOneElementOf(e, right))
      }
    }
    ...
    def contain(resultOfKeyWordApplication: ResultOfKeyWordApplication)(implicit keyMapping: KeyMapping[T]): Assertion = {
      doCollected(collected, xs, original, "contain", outerStackDepth) { map =>
        ...
        else indicateSuccess(FailureMessages.containedKey(map, expectedKey))
      }
    }
    ...
    def contain(resultOfValueWordApplication: ResultOfValueWordApplication)(implicit valueMapping: ValueMapping[T]): Assertion = {
      doCollected(collected, xs, original, "contain", outerStackDepth) { map =>
        ...
        else indicateSuccess(FailureMessages.containedValue(map, expectedValue))
      }
    }
    ...
    def startWith(right: String)(implicit ev: T <:< String): Assertion = {
      doCollected(collected, xs, original, "startWith", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.startedWith(e, right))
      }
    }
    ...
    def endWith(expectedSubstring: String)(implicit ev: T <:< String): Assertion = {
      doCollected(collected, xs, original, "endWith", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.endedWith(e, expectedSubstring))
      }
    }
    ...
    def include(expectedSubstring: String)(implicit ev: T <:< String): Assertion = {
      doCollected(collected, xs, original, "include", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.includedSubstring(e, expectedSubstring))
      }
    }
  ...
  sealed class ResultOfBeWordForCollectedAny[T](collected: Collected, xs: scala.collection.GenTraversable[T], original: Any, shouldBeTrue: Boolean) {
    ...
    def theSameInstanceAs(right: AnyRef)(implicit toAnyRef: T <:< AnyRef): Assertion = {
      doCollected(collected, xs, original, "theSameInstanceAs", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.wasSameInstanceAs(e, right))
      }
    }
    ...
    def an[U <: T](beTrueMatcher: BePropertyMatcher[U])(implicit ev: T <:< AnyRef): Assertion = { // TODO: Try supporting 2.10 AnyVals
      doCollected(collected, xs, original, "an", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.wasAn(e, UnquotedString(beTrueMatchResult.propertyName)))
      }
    }
    ...
    def definedAt[U](right: U)(implicit ev: T <:< PartialFunction[U, _]): Assertion = {
      ...
      if (e.isDefinedAt(right) != shouldBeTrue)
        ...
        else indicateSuccess(FailureMessages.wasDefinedAt(e, right))
      }
    }
...
*/
    ...
    def should[U](inv: TripleEqualsInvocation[U])(implicit constraint: T CanEqual U): Assertion = {
      doCollected(collected, xs, original, "should", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.equaled(e, inv.right))
      }
    }
    ...
    def should(inv: TripleEqualsInvocationOnSpread[T])(implicit ev: Numeric[T]): Assertion = {
      doCollected(collected, xs, original, "should", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.equaledPlusOrMinus(e, inv.spread.pivot, inv.spread.tolerance))
      }
    }
  ...
  final class ResultOfHaveWordForCollectedExtent[A](collected: Collected, xs: scala.collection.GenTraversable[A], original: Any, shouldBeTrue: Boolean) {
    ...
    def length(expectedLength: Long)(implicit len: Length[A]): Assertion = {
      doCollected(collected, xs, original, "length", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.hadLength(e, expectedLength))
      }
    }
    ...
    def size(expectedSize: Long)(implicit sz: Size[A]): Assertion = {
      doCollected(collected, xs, original, "size", outerStackDepth) { e =>
        ...
        else indicateSuccess(FailureMessages.hadSize(e, expectedSize))
      }
    }
    /**
     * Overrides to return pretty toString.
  ...
  sealed class AnyShouldWrapper[T](val leftSideValue: T) {
    ...
    def should[U](inv: TripleEqualsInvocation[U])(implicit constraint: T CanEqual U): Assertion = {
      ...
      else indicateSuccess(FailureMessages.equaled(leftSideValue, inv.right))
    }
    ...
    def should(inv: TripleEqualsInvocationOnSpread[T])(implicit ev: Numeric[T]): Assertion = {
      ...
      else indicateSuccess(FailureMessages.equaledPlusOrMinus(leftSideValue, inv.spread.pivot, inv.spread.tolerance))
    }
