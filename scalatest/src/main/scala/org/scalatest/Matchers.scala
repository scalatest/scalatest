    ...
    def shouldEqual(right: Any)(implicit equality: Equality[T]): Assertion = {
      doCollected(collected, xs, original, prettifier, pos) { e =>
        if (!equality.areEqual(e, right)) {
          val (eee, rightee) =
            equality match {
              case differ: Differ[T] => differ.difference(e, right).inlineDiff.getOrElse(Suite.getObjectsForFailureMessage(e, right))
              case _ => Suite.getObjectsForFailureMessage(e, right)
            }
  ...
  sealed class AnyShouldWrapper[T](val leftSideValue: T, val pos: source.Position, val prettifier: Prettifier) {
    ...
    def shouldEqual(right: Any)(implicit equality: Equality[T]): Assertion = {
      if (!equality.areEqual(leftSideValue, right)) {
        val (leftee, rightee) =
          equality match {
            case differ: Differ[T] => differ.difference(leftSideValue, right).inlineDiff.getOrElse(Suite.getObjectsForFailureMessage(leftSideValue, right))
            case _ => Suite.getObjectsForFailureMessage(leftSideValue, right)
          }
